...
首次搭建按固定顺序完成：填充运行配置与主线入口提示词匹配规则，使用项目已有运行时实现 `governance-cli` 与 Hook 适配器，复制 Schema 和 Governor 配置，运行 `doctor`，再启用并信任 Hooks。状态不存在时，普通会话保持未激活；只有 `UserPromptSubmit` 命中主线入口提示词才原子执行 `init` 与结构化首次激活复核。治理状态存在后，真实 `SessionStart` 或再次提交完整规范主线提示词才能恢复复核；普通消息和普通回复结束均保持沉默。由此避免讨论和支线任务被治理抢占，同时保证主线首次启动、恢复和完成后再次打开项目都有唯一行为。
...
- 每个真实 `SessionStart` 实例原子对应一个结构化固定触发；同一实例重放只复用同一请求。已有合法 Review 待处理时，恢复边界只重现该请求，不替换或增加代次；已应用结论不能替代后来出现的全新恢复实例。
...
`events.jsonl` 只增记录状态转换、证据身份与摘要、可验证失败事件、修复代次、资源边界事件、复核请求和结论；它用于审计与恢复，不是第二份状态。不得复制原始工具输出、终端流水、会话文本或可由引用产物读取的内容；需要保留的原始结果另存为证据文件，事件只记录路径和哈希。耗时可以作为 Governor 复核时的事实，但不能单独触发停止、否定路径或限制开发总时长。
...
主任务首次明确激活以及 `startup`、`resume`、`clear`、`compact` 的真实 `SessionStart` 实例必须创建结构化固定复核；同一实例只能创建一次。已有合法 Review 待处理时只重现同一请求，不能用新的说明文本替换它。在 Governor 结论通过确定性校验并落盘前，主 Agent 只能恢复事实、读取复核输入、启动并等待 Governor，不得继续产品实现、批量处理或正式验证。

固定复核必须让 Governor 直接读取目标、完成条件、仓库事实和原始证据；`state.json` 只负责指出当前声称的进度、路径和证据位置。状态与事实不一致时以事实为准，并局部失效错误状态。

Governor 是独立子会话。治理模板只把真实 `SessionStart` 作为主任务恢复边界，不注册通用 `Stop`，也不注册 `SubagentStart` 或 `SubagentStop`；因此普通回复结束和 Governor 启停都不会创建固定复核。不得在 `governor.toml` 中关闭 Hooks；Governor 保持只读，其读取动作由现有低成本读取路径快速放行，其他安全、审计和项目 Hook 继续正常运行。
...
2. 同一归一化失败在一次绑定了仓库、候选、配置、运行身份和已验证证据的针对性修复后，由新的真实执行事件再次出现；同一事件重放或同一修复代次内重复均不触发；
...
| `hook <event>` | 消费 Codex Hook 标准输入并确定性映射：`session-start` 以 Hook 实例创建或重现固定复核；`user-prompt-submit` 默认沉默，只处理首次激活、完整规范主线提示词恢复、精确用户事项和交接；`pre-compact` 校验状态已原子落盘；`pre-tool-use` 对纯读取快速放行，否则校验批准与范围；`post-tool-use` 只登记可信事实事件。兼容 `hook stop` 严格空操作且不得注册到新配置 |
...
| `record-repair` | 引用当前工作包内已验证失败事件和该事件发生后新登记的验证证据；仓库、候选、配置与实际执行程序身份由运行时自行计算并确认发生变化后，幂等推进该签名的修复代次，调用者不能自报身份 |
| `request-advisory-review` | 以显式稳定 `request-id` 创建 advisory；原因文字只用于说明，不能声明固定或事实事件 |
| `request-completion-review` | 绑定调用身份、当前权威和完成证据摘要，创建结构化完成候选事件；不接受自由文本事件类型 |
| `migrate-invalid-stop-review` | 只精确迁移活动 `event:main-agent-stop`，归档错误请求和结果、保留真实进度并创建一次恢复复核；普通 Hook 不得自动调用 |
...
`governed-run` 只用于需要独立进程存活保护的外部命令，不限制正常开发时长。它依据命令自身提供、官方说明或最小探测确认的心跳、输出或检查点信号判断进程是否仍然存活；只有进程丢失既定存活信号时才终止完整进程树、保存已有结果并以本次执行身份自动登记失败。命令持续产生有效存活信号和必要证据时，无论总时长多久都不得因计时停止。普通读写、编辑和定向测试不经过该包装器。PostToolUse 同样只用 Codex 提供的真实 `tool_use_id` 登记；调用者不能自造固定或事件身份，主动复核只能明确创建 advisory。
...
| 事件 | 行为 |
| --- | --- |
| `UserPromptSubmit` | 默认快速无输出；状态不存在时只对完整主线入口提示词执行首次激活，已有 run 时仅同一完整提示词显式恢复同一请求或创建一次 typed resume；精确待用户事项和交接按身份处理。普通消息、complete 和非所有者状态均不重复注入治理文字 |
| `SessionStart` | 状态存在后，在 `startup`、`resume`、`clear` 和 `compact` 时以 Hook 实例创建固定复核；已有合法 Review 只重现同一请求。状态不存在时不建立主线任务，非所有者只返回一次最小只读事实 |
...
主线入口必须由 `UserPromptSubmit` 的项目特定完整匹配规则明确激活；匹配规则只识别持续开发主线，不得覆盖普通对话或支线。结构化触发分为固定、事实事件和 advisory：类型、来源身份与实例 ID 决定授权，原因文字不参与授权。合法来源边界原子创建请求并向主 Agent交付一次；后续普通消息和回复结束不得补发、抢占或制造新请求。`PreToolUse` 在待复核时继续拒绝产品修改，但不创建 Review 或启动 Agent。完成保护只走 `request-completion-review → task_complete → finish`，不使用 `Stop` 推测任务完成。
...
1. **安装与激活**：确认权威层和完成定义充分并执行 `doctor`；由主线入口提示词首次命中时原子执行 `init` 与首次结构化固定复核，普通任务不建立状态。
2. **固定复核**：主线激活后，每个真实恢复、清空上下文或压缩实例只创建一次固定复核；已有待处理请求只重现同一身份，复核完成前不继续产品工作。
...
- 主线激活后，恢复、清空上下文和压缩的每个真实实例均会固定复核，复核完成前产品动作被阻止；已有待处理请求只重现同一身份；
- 连续发生两个不同的启动或压缩实例会形成两个固定触发实例；同一实例重入不会重复创建请求，同一已消费事实事件也不能再次创建；
- running、Review 待处理、已批准、complete 和非所有者五种状态下，连续普通消息与兼容 `hook stop` 均无上下文注入和状态副作用；非所有者产品写入仍被拒绝；
- CLI 不存在自由文本 `resume` 或 `request-review`；固定和事实事件只接受受限结构化来源，显式 advisory 不会被普通 Hook 重试或升级；
...
- 无既有状态的 `SessionStart` 保持未激活，主线 `UserPromptSubmit` 能够原子初始化并进入固定复核，运行中只有完整规范主线提示词显式恢复，完成状态不会被覆盖；Hook 模板不注册 `Stop`，全部子命令与 `hook <event>` 契约一致；
- 旧 `event:main-agent-stop` 只能由显式受限迁移命令归档；迁移在各阶段中断后可重入，保留所有者、工作包、条件、证据与失败历史，并只创建一个恢复复核；
...
- 完成证据不足时 `finish` 机械失败；用户主动打断和普通回复结束不会被治理 Hook 阻止；
