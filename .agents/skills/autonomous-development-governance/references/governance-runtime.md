...
`governed-run` 只用于需要独立进程存活保护的外部命令，不限制正常开发时长。它依据命令自身提供、官方说明或最小探测确认的心跳、输出或检查点信号判断进程是否仍然存活；只有进程丢失既定存活信号时才终止完整进程树、保存已有结果并以本次执行身份自动登记失败。命令持续产生有效存活信号和必要证据时，无论总时长多久都不得因计时停止。普通读写、编辑和定向测试不经过该包装器。PostToolUse 同样只用 Codex 提供的真实 `tool_use_id` 登记；调用者不能自造事件身份，需主动复核时使用 `request-review`。
