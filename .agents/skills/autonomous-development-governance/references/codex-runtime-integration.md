...
2. 将本机 Codex CLI 更新到 `0.149.0`，通过官方 Hook 管理界面审阅并信任当前 `.codex/hooks.json` 的四个治理 Hook：`SessionStart`、`UserPromptSubmit`、只匹配 Governor 的 `SubagentStart` 与异步 `SubagentStop`。`SessionStart` 必须覆盖 `compact`，因为该事件能在压缩后的下一次模型请求前交付 `additionalContext`。不得注册工具、压缩前或通用 `Stop` Hook；治理不参与工具许可，回复结束也不代表复核边界。
3. 使用全新会话而不是当前会话或脚本直调进行冷启动验收：确认稳定启动标识与真实恢复边界只注入一次顾问复核，目标模式重复提示复用同一触发，连续普通消息保持沉默；主 Agent 启动 Governor 后无需等待，`SubagentStart` 绑定其身份，异步 `SubagentStop` 自动接收 JSON；Governor 配置无 `malformed agent` 警告，且没有第二个 Ownward MCP 进程；Governor 不可用时主线仍能继续。
4. `doctor` 必须机械检查 Governor 的只读顾问属性、治理 Hooks 只包含固定边界和异步结果接收、没有工具或 Stop 门禁，以及项目存在 Ownward MCP 时 Governor 同名 MCP 段包含完整 transport 并明确禁用；同时以隔离夹具验证普通消息、幂等触发、显式回应和失败开放。
5. Windows 的运行期 Hook 命令必须通过 `cmd.exe /d /c` 调用轻量 `.cmd` 启动器；不能把带引号的 `.cmd` 路径直接置于命令首部。安装后必须用真实 `UserPromptSubmit` 载荷证明 Codex 实际执行了 Hook，不能只验证配置能解析或脚本可手动运行。

## 最终说明

Governor 重复 MCP 与 Hook 未自动加载的问题已经完成独立修复。通用 `Stop` 会把每轮回复误判成复核边界，工具 Hook 又无法可靠证明主目标归属和结果合同，因此现行配置只保留 `SessionStart`、`UserPromptSubmit` 及只匹配 Governor 的 `SubagentStart`、异步 `SubagentStop`；固定边界自动创建请求，父子生命周期自动接收结果，普通消息不注入治理动作，Governor 能正常加载且不会启动第二个 Ownward MCP。
