...
Codex 插件随 Windows 发布包交付，安装方式见[项目使用说明](../../../README.md#codex-integration)。插件使用原生 `mcp_tool` Hook 调用已连接的 `ownward_host_event`，此工具仅属于宿主连接器，不进入内核契约。材料核对沿用原主体，不另起模型；可选组织执行器由连接器单独管理，Hook只传递前台活动信号。插件启用后仍需完成宿主的正常信任确认，不解析对话文件。[官方 Hook 契约](https://learn.chatgpt.com/docs/hooks)。适配如下：
...
| `UserPromptSubmit`、`Stop`、`Interrupt`、`SessionEnd` | 可选组织执行器据此前台状态停止派发或继续取件；Hook自身不运行AI，也不等待整理完成。 |
...
另行设置`OWNWARD_INFORMATION_USE_PATHS=v1`可启用同一入口的快慢取用契约；正式连接器在MCP初始化时提供规则，模型在已有任务中选择，不另起分流调用。接续沿用当前任务和原生累计用量，跨轮核对仍由上述Hook负责；变化状态进入正常任务输入，失效内容及依赖结论停止作为依据，受控材料集清除不可用项，不承诺擦除Codex历史对话。取消变量后重连恢复原规则，数据与待办不变。

该开关与组织执行器同时启用时增加`ownward_organize(id)`：当前主体须有维护权限，执行器仍须通过原资源准入。最多保留32个依赖资料身份，沿同一待办优先领取；已有执行者继续原工作，不抢占凭据、不增并发。返回`requested`只表示已请求，完成用`ownward_status`核对；无资源时如实返回不可执行。

依赖优先资格仅属于当前活动任务，任务结束、清空或开始下一任务时撤销；派发前重新核对，其他会话的有效依赖不被清除。连接内无法唯一确认活动任务时返回不可优先执行，保留普通后台待办。预算耗尽事实保存在有界执行元数据中，普通状态查询及再次请求会返回原因与`restore_organization_execution`，不继续表示可自动恢复；按体系、主体授权修订及当前`execution_identity`核对，旧失败不覆盖新来源或其他执行者已完成的结果。

Windows正式连接器可按 `OWNWARD_ORGANIZATION_PROFILE` 指向的本机绝对路径读取JSON部署配置；不配置时不启动执行器、不改变原写入。独立组织使用已验证的原模型、档位、材料、组织规则和恢复预算；部署方须确认预留容量与前台限流池独立，不能把另开进程当成资源隔离，也不要求另购模型套餐。不具备这一条件时保持原路径，不能把降级计作后台能力通过。

| 配置 | 含义与边界 |
| --- | --- |
| `executable`、`home` | 官方Codex可执行文件和组织专用配置目录的绝对路径；由宿主正常管理认证，不复制主会话。目录不得配置其他MCP、插件、命令通知或外部模型指令文件。 |
| `model`、`provider`、`effort` | 已验证组织能力的原配置，任务创建后核验模型身份，禁止自动换模型。 |
| `reservation` | 已核实的独立资源预留标识；涉及多机器时不得重复分配同一预留。本字段是部署资格声明，不是自动测得的服务端额度。 |
| `memory_mib` | App Server完整进程树的内存硬上限，64—2048 MiB；运行前还须保留至少512 MiB或物理内存10%的空闲余量，取较大者。 |
| `timeout_seconds`、`max_submissions`、`max_attempts`、`max_tokens` | 沿用原验证条件，分别约束单次任务时长、每次提交数、同一工作跨重连总尝试数和累计已知Token；上限分别为1800秒、10次、3次，Token须为正数。不得为启用后台放宽原预算。 |

执行器启动时先验证App Server协议、独立线程与工具契约，不调用模型；当前验证宿主为Codex CLI 0.149.0。通过后才自动为未指定组织方式的新写入选择延后协议；重试沿用第一次写入的方式。原文可立即读取；结束前台响应后从内核持续领取，重连无需用户手工唤醒。跨连接共享本机一个执行名额、前台活动状态及用量记录；压缩不解除仍在执行的前台任务，内核再限制一个后台持有者。领取前先核对可信接替地址，再检查主体权限；远端会话失效时有界重连，领取与提交身份不变。正常续期沿用当前会话，不等待慢工作结束或逐次刷新路由。正在执行的同一待办不重开，依赖者复用其完成结果；未执行的无关后台工作让位于前台。

每次只取得当前主体获准的组织材料，使用独立临时上下文；单条工作消息上限8 MiB，超出时保留待办与原文。唯一模型工具为原 `ownward_semantic_submit`，无Shell、管理、其他MCP、技能或Hook执行权限。每15秒续期，提交前重新核对工作材料；权限、版本或世代变化时旧结果拒绝。内核接受语义即结束模型任务，向量待补不触发模型确认。正常退出先停止执行器并释放凭据，再关闭共享通道、回收所属完整进程树与临时目录；恢复重新领取有效材料，已被内核接受的语义不重做，向量未完成只续补向量。

失败受原预算约束，保留待办并继续其他资料。宿主仅在用户缓存目录 `Ownward/organization/execution.sqlite` 保存身份、尝试次数、用量和状态，无原文副本、无第二份业务队列；元数据库最多约16 MiB，成功记录最多保留256条，未完成记录不能因清理而重置预算。模型完成响应数与已报告Token是可观测用量；中断或传输结果不明另作标记，包括启动请求发出但未收到轮次ID的情况；不当作零成本，仍占一次尝试。Token限制在宿主收到用量时检查，不承诺在途调用绝不超出阈值。达预算或元数据容量上限时停止相应新调用，原文继续可用。

实际入口：[连接器接续](../../../cmd/ownward/organization_host.go)、[App Server适配](../../../internal/codexplugin/organization.go)。本单元验证执行机制与资源边界，真实模型质量、总成本及快慢路径整体时效仍按[任务验收](../../tasks/information-availability-and-on-demand-processing.md#单元三快速取用与整体交付)执行；新增路径当前不默认启用。
