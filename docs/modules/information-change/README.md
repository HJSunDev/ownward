...
快速取用路径提供`ownward_organize(id)`（当前主体须有维护权限）：领取指定待办并把工作材料交回调用者——返回租约、组织材料与期望提交结构；已有持有者时返回持有状态与接续信息，不唤醒后台、不抢占凭据、不增并发。调用者完成组织后经同一提交契约提交，用`ownward_status`核对；待办与权限约束不变。

预算耗尽事实保存在有界执行元数据中，普通状态查询及再次请求会返回原因与`restore_organization_execution`，不继续表示可自动恢复；按体系、主体授权修订及当前`execution_identity`核对，旧失败不覆盖新来源或其他执行者已完成的结果。
