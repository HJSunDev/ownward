...
实现[跨地点访问与便捷迁移](../../product/cross-location-access.md)；产品授权复用[用户控制架构](../information-control/README.md)，状态与实施进度由[工作包二](../../tasks/information-control-and-change.md#工作包二跨地点访问与便捷迁移)维护。本文是实现依据，不是已交付声明。
...
工作包一继续决定谁能做什么；工作包二决定怎样到达同一权威、怎样接续和交接；工作包三决定信息变化如何供后续使用核对。共同使用体系身份、主体、资产版本与停止使用状态，不建立平行权限库、资产版本或语义规则。
