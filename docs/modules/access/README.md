...
`ownward owner-window --data-dir <库目录>` 通过既有 connect-or-start 找到同一回环服务，复用系统账户保护的物主验证，打开一次性引导链接。浏览器交换引导令牌后只持有短期设备会话；原物主凭据不进入网页。服务端每次 API 校验物主身份、精确 Host／Origin 与版本头，机器控制路径继续拒绝浏览器 Origin。令牌单次有效 60 秒，最多 32 个设备会话、各 12 小时；会话仅存浏览器 sessionStorage，恢复或撤销立即失效，无业务副本。

库目录的 `owner-window.json` 是无秘密、可重建的入口描述，不是权威或凭据。每次监听启动、目的地激活及源退役由当前位置／Handoff 生成；发现入口时按实际共享服务及耐久 Handoff 核验，失效描述重建，退休源只指向可信目标。备份和迁移不复制旧机器的回环地址或设备会话，目标启动生成当地入口；远程浏览器首验仍在后续范围。运行物主入口命令后打开内嵌五面网页；网页不要求智能体在线，关闭不影响体系运行。窗口正文与异常输入在认证失效时隔离，重验后向权威核对；明确退出清除会话及窗口暂存。

物主决策与宿主确认读取同一 management／enrollment／Handoff 记录。接入卡包含目标证明绑定的公开核对标记；迁移批准归原 Handoff、冻结前核验批准者当前修订，失效批准重新呈现。任一路有效决定取消另一处等待并接回原任务，迟到表单不能覆盖已生效决定。智能体通过标准 MCP `ownward_draft_work` 读写指定授权草稿，无须任何全库读取权限，也不能自行发布或枚举其他草稿。

拒绝迁移在同一权威提交中保存原 ID 终态并释放活动占位；拒绝与显式取消均由服务唤醒既有清理循环，不依赖原宿主在线。旧请求重试只读取已有终态，迟到取消只处理对应 ID，不能改动新迁移。清理与迁移写入互斥；锁忙时保留耐久待办并退避重试，不阻塞持锁迁移关闭运行时，完成清理后才标记副本已清除。

接入批准在目标确认领取前失去有效性时，同一申请重新呈现待确认；实际批准者和接收主体的当前修订参与决定版本，旧句柄或旧表单不能重新批准。公开核对标记仍绑定原目标证明，与决定版本分离。已经签发但未确认领取时保持同一主体，重新批准不能恢复已撤销权限；已确认领取的申请不重开。

宿主迁移表单固定呈现时取得的决定版本，后台轮询不能替它换成新批准资格；失效后需重新呈现确认，同一迁移身份可继续。接入与迁移决定的公共事实在原权威提交事务内进入既有事件账本，与普通管理记录合并形成有界只读历史；动态读取同一事实。领取重试、ACK、冻结和清理不重复记批准；旧导入记录不补造发生时间。历史仅保留对象显示、权限、决定结果与时间，不含凭据、证明或证书，不可用于再次批准；附属记录随账本过期，不删除权威防重放及清理义务。
...
`service-approve --data-dir <服务位置>` 列出待接入预览；核对后以 `--id`、`--marker`、`--decision <预览中的 enrollment.decision>` 和 `--approve=true` 批准（拒绝用 `--approve=false`）。决定始终提交原预览版本，失效后重新查看与确认；命令不会在提交时替用户换成新版本。
...
目标先提出申请时，绑定连接材料中已有的管理接收连接，在其下一次 Ownward 调用中带回；没有接收连接时，使用上述可信宿主发起路径关联同一申请，不能声称已经通知所有者。普通连接提出其他管理请求沿用同一交接与回执机制，只有提案权；未配置或暂不可达时准确表达待批准，保留原请求而不要求重新描述任务。所有确认与领取均重查当前权限；申请有界保留，申请取消或过期后不自动重建。批准身份失效但尚未确认领取的有效申请，重新确认的仍是原申请。
...
**资产创建、更新及批量变更的去重事实与对应变更在同一权威事务中提交，不另建需要双写的回执库。** 操作身份、输入摘要、已提交的变更及逐项结果元数据一同耐久成立后才报告成功。现有日志格式通过同一条 `assetlog` 记录实现；[存储重设计](../storage/bounded-runtime-design.md)通过同一SQLite事务实现。恢复只接受完整已提交状态，未完成写入不生效，损坏报错。成功提交但响应丢失时恢复原回执；未提交则不存在半份成功，调用方仍使用原标识接续。提交结果不确定时，先停止该存储的新提交并恢复核对，不能盲目重做。不识别新存储格式的旧程序必须拒绝打开。

权限与版本核对仍在既有短提交边界内完成，更新不绕过预期版本。管理决定继续与其回执保存在同一控制提交中；语义工作复用工作身份和世代接受记录，权威控制与派生状态的逻辑职责不因物理存储合并而改变。批量变更保留原有逐项结果语义，同次提交的成功项与回执整体恢复；资产成功后派生更新中断，补的是已有资产的派生工作，不能重新创建资产。
