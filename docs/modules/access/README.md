...
库目录的 `owner-window.json` 是无秘密、可重建的入口描述，不是权威或凭据。每次监听启动、目的地激活及源退役由当前位置／Handoff 生成；发现入口时按实际共享服务及耐久 Handoff 核验，失效描述重建，退休源只指向可信目标。备份和迁移不复制旧机器的回环地址或设备会话，目标启动生成当地入口；远程浏览器首验仍在后续范围。本单元仅交付端口与验证落地页，五面网页由单元三交付，入口须显式启用。
