...
图能力的目标设计见[关系组织架构](../semantics/graph-organization.md#检索关联必须进入可用材料)：关系必须转化为有界、可核对的原文材料，来源校验与直接检索保持统一；实现状态由[图能力工作包](../../research/graph-value-and-cost.md#工作包图能力设计实现与验收)维护，不以设计完成替代下方运行版本的验证记录。
...
| 尚未证明 | 当前候选完整验收、固定专项表现、官方清洗 LongMemEval‑S 公开成绩及相对其他编排的最优性 |
...
第一版完成前，必须在接入最终向量能力后，以同一候选版本完成内核前沿优化环、固定 Ownward 专项数据集和官方清洗 LongMemEval‑S 验证，同时比较检索质量、检索时效与资源成本。结果支持 V1 时补充其最终证据；只有现有证据无法收敛且具体替代方案可能改变完成结论时，才复用同一批固定材料执行最小定向对照。需要改变编排时新增本模块的检索编排 V2，写明相对检索编排 V1 的变化、验证表现与取代依据，不改写 V1。
