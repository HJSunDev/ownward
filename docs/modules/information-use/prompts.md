...
**格式纠错：** 合法整数与单项位置列表按Schema作无歧义转换；精确字段补交可去除唯一可识别的额外包装，完整JSON后多出的闭合括号可清除，内容和位置不变。其余不合格字段由外部智能补交，其他字段原样保留，合并后仍须通过完整格式校验及原文引用检查。固定批次数组未闭合时，保留已完整返回的条目；补交失败或超时也保留已合格资料，使用剩余纠错次数处理缺口。无法可靠解析的内容不得接受，纠错次数及超时上限不增加。
