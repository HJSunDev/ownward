...
1. **固定任务依据：** 外部智能先只看原始需求，列出完成它所需的最小事实与关系；代码将这些要点设为后续交付的对应字段，避免相似资料悄悄改变任务，分解本身不充当事实。
...
4. **一次组织交付：** 外部智能明确用户要达成什么、原文支持什么及哪些关联未确定，再形成可用结果；只有存在实质不同且有据的解释时，才保留相应条件结果。代码原样组合结果，不追加模型复核、不以确定性规则推断事实。

标准流程包含一次独立任务准备与后续取证交付；“一次交付”不限制工具轮数或正常思考。已有原始材料的宿主可使用`respond`完成任务准备与信息交付，无须重复检索。
...
已有工具循环的宿主先按`FRAME`与`FRAME_SCHEMA`独立明确任务依据，用`task_contract(frame)`生成交付指令和结构，再以`initial_context(query, call)`准备线索、继续自主取证，最后调用`finish`交付。只处理已获材料时，`respond(observations, invoke)`依次完成任务准备与信息交付；`invoke(stage, instruction, payload, schema)`由宿主实现，不隐式继承其他任务或历史答案。组件不需要知道模型来自轻量客户端、Codex还是其他运行环境。
