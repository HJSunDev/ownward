...
- **职责边界稳定**：体系只直接服务外部智能体；主动检索中的语义决策由外部智能体承担，体系不承担用户交互，也不内置替代它的智能体。
...
| 轻量运行 | 总体结构只保留产品不可缺少的边界；派生状态和处理链路按可验证的质量与时效收益建立，不以通用性或未来可能增加常驻复杂度。 |
...
7. 主动检索中的语义决策由外部智能体承担，体系不内置替代外部智能体的检索智能。
8. 检索技术、模型、索引和组织算法均可替换，任何一种都不能成为信息资产的唯一解释者。
