...
2. **另一方案：** 将原文、理解结果与初稿交给一次新调用，提出有原文支持的其他解释及可用方案。
...
**默认入口让外部智能在现有响应中同时给出当前工作与是否使用组件的选择。** 直接完成不增加调用；选择组件则将当前工作作为初稿复用，从理解步骤接续，不重复起草。无初稿的完整流程为5次调用，有补充则6次；默认入口包含首次响应也是5次或6次。这些次数不含此前检索过程，内部步骤可独立演进，调用方无须依赖步骤数量。
