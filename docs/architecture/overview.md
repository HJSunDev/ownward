...
“资产永久”服从用户意愿：授权访问、撤销权限与主动遗忘均由唯一权威执行，不能因内核、接入或世代替换而绕过。

Ownward 是与通用智能体平级的个人信息基础设施，不是某个智能体的记忆组件、模型包装层或“笔记加检索”。它由用户长期持有，供现在和未来任何形态的智能体通过同一核心共同使用和补充；外部智能体承担任务对话与语义操作，物主窗口提供同一权威的可视投影、纯文字工作面与掌控入口。

Ownward 不追求“一切皆插件”或任意代码热插拔。真正需要的是：当前有效能力持续可用，候选实现能够独立建立、验证、晋升和回退；替换不能改变用户信息、制造平行真相或迫使无关能力重新验证。

**信息获取与信息使用是两个独立的能力边界。** 信息内核维护、组织和交付信息；信息使用层提供组织上下文、衔接工作阶段与交换依据的协作能力，使接入智能体更好地使用已获信息。任务安排与语义判断始终由外部智能体以自身智能执行；Ownward 不为此内置模型或代行任务的智能体。能力通过统一契约提供，不绑定测试运行器、模型或题型。

## 二、长期原则

架构设计遵循[项目哲学](../philosophy.md)，将产品对用户的体验承诺落实为内部职责与稳定契约。
...
7. **体验约束结构**：先确定用户应能完成什么、只需作出哪些决定，再设计内部协作；既有实现不能反向变成用户必须理解的概念或操作步骤。
8. **独立变化分别表达**：使用主体、可执行能力、作用范围与有效条件等能够独立变化的业务关系应保持清晰边界，按真实需求组合，不与智能体角色标签、设备形态或具体场景绑死。仅实现已经明确的需求，不为穷举未来组合预建分支或通用框架。
...
| 信息使用层 | 提供任务材料、上下文及工作反馈的协作支撑，帮助外部智能体有效安排信息使用过程 | 内置模型或代行用户任务的智能体、按题型硬编码语义、把中间推断写成权威事实 |
| 派生世代 | 保存特定内核可重建的关系、表示和索引 | 成为信息权威或承载不可重建语义 |
| 外部智能能力 | 提供开放内容理解和语义表示 | 直接写资产、决定组织权威或拥有用户信息体系 |
| 统一能力契约 | 稳定表达检索、读取、创建、更新、语义协作、规则获取与信息使用 | 暴露内部存储、算法、模型或部署方式 |
...
具体组织以有原文依据的证据单元、对象线索和可修正关系支持跨资料取证，保持对象、事件及条件边界；表示与生命周期由[关系组织架构](../modules/semantics/graph-organization.md)规定，不改变长期资产和统一语义权威。
...
| 物主工作状态 | 权威运行基座中的独立私有域 | 草稿耐久保存、随库备份迁移；发布时原子转为资产，弃稿或绑定目标被遗忘时清理 |
| 能力派生世代 | 确定的内核或模型世代 | 可并存、验证、晋升、回退和回收 |
| 运行状态 | 当前进程与请求 | 可丢弃，不承载唯一事实 |

权威控制状态保持最小，只保存无法从资产和装配清单安全推导的产品决定。任何新状态必须明确归入一类；无法归类意味着职责边界尚未成立。

物主工作状态与资产同库同事务，但不进入普通读取、检索、导航或语义工作。智能体只能通过物主授予的草稿工作项读写指定版本，不能自行发布。操作账本与资产／管理决定同事务记入无正文的事件引用；呈现历史有界，裁剪呈现不得删除仍有效的授权、删除屏障或防重放依据。物主窗口分期交付与边界见[模块设计](../tasks/owner-window.md)。

物主视图端口 `ownward.owner-view/v1` 与 MCP 接入平级，薄投影直接读取同一权威与派生契约；本机入口复用原回环监听与物主验证，不新增智能、常驻进程或业务存储。标准 MCP 的限域草稿工具与浏览器条件提交共用工作状态契约；恢复、迁移和双轨批准沿原权威流程。五面网页以原生 JS 与 CSS 内嵌单二进制，无前端构建或外部资源依赖。只读定位引用支持重连后核对同一稿件，不能充当权限或写入条件；整篇流式替换仍在原草稿事务中条件提交。异常输入暂存不构成第二权威，遗忘失效后清除正文，未知发布仅保留原操作核对线索。

持久化词法、向量和图索引归入能力派生世代；运行状态仅保留有界缓存和任务工作区。正常启动不以全量读取资产或重建索引为前提，具体落点见[存储实施设计](../modules/storage/bounded-runtime-design.md)。
...
失败归因、修复范围与停止投入的依据统一遵循[产品职责边界](../product/requirements.md#职责边界)，不因外部智能体的能力或价值取向差异扩展内核职责。
...
用户控制能力复用该基座管理授权与遗忘：认证由接入层完成，产品授权和资产生效状态由权威基座决定，派生清理由内核及存储执行。撤销须约束后续提交与交付；遗忘先耐久停止使用，再完成受控副本清理，中断、旧工作和世代回退不能恢复已停用信息。具体职责、共享契约与恢复边界见[用户控制架构](../modules/information-control/README.md)。

信息体系及接入主体的身份随权威状态保存，不由设备、目录或网络地址生成；所有者证明与连接建立由部署适配提供，位置变化不能重新定义所有权或授权。正常接续沿用当前权威状态，恢复历史快照按独立恢复契约处理。

部署迁移由权威基座耐久交接活动资格，先停用源位置再启用已验证的目的地；认证、位置发现和连接恢复止于接入适配。迁移保留当前资产、授权及兼容派生结果，不等同于恢复历史快照或升级内核；中断不得产生两个活动权威。

管理操作通过宿主侧适配完成请求、必要批准和原任务接续，权威层持有实际执行与完成状态。遗忘的前台生效、受控副本清理及后续语义重建分别完成：用户不等待后台整理，体系不以旧材料或遗漏清理换取可用性。

信息变化沿用同一资产、版本和停止使用状态：变化的含义与必要历史保存在资产中，读取依据绑定实际交付的来源，确定性能力核对来源是否变化，宿主在重新使用材料时按需接续。来源一致性不替代语义适用性或新信息检索；理解和行动仍由外部智能体完成，不建立平行版本库或跨外部行动的长事务。具体边界见[信息变化架构](../modules/information-change/README.md)。
...
**信息使用层把目标、事实依据、未决关系和有条件的结果组织到外部智能的同一次交付中。** 理解、推理、适用性判断与取舍始终由接入方自己的智能完成；代码提供信息交接与结果结构，不判断开放语义。

默认深度取用流程保持如下：

1. **准备任务依据：** 外部智能先只看原始需求，按日常含义列出所需的最小事实与关系；保留用户指定的对象、事件及操作，初步解释不充当事实，也不能替代用户需求。
2. **取得初始线索：** 以原始需求调用内核检索，并尝试读取前三条返回来源的首个证据片段；经过宿主原有权限、读取预算和留痕，不使用答案来源标签。
3. **按需继续取证：** 将原需求、正式规则和初始材料交给外部智能，它仍可多轮调用公开工具补足所需信息；初始线索不代表资料完整，不替它决定何时信息足够。
4. **形成结果：** 外部智能根据已有依据、未决关系及必要条件判断任务完成状态，再形成主结果与有据的条件结果；不重复输出逐项依据清单。
5. **交付结果：** 代码原样呈现外部智能给出的主结果及条件结果，原文依据保留在取证轨迹中，不根据完成状态筛选结论。

可选快速路径复用智能体已有任务理解，按需读取或检索，不执行独立需求准备与固定初始读取；综合、冲突或范围不明时直接深度处理。快速不足时携带有效原文及`basis`、必要说明、原始需求与累计用量接续，只补缺少的步骤；Python接续剔除失效材料及依赖笔记后构造新输入；Codex原生接续将变化状态注入正常任务，停止沿用失效依据并重读，插件不擦除宿主历史。两者均不重置预算。已有向量与图照常可用，路径选择、证据充分性和交付内容均由外部智能判断。接口与显式启停见[信息使用接入](../modules/information-use/README.md#可选快速取用与接续)。

标准流程包含任务准备、取证并形成结果两个外部智能阶段，取证阶段可多轮调用工具；原任务的必要上下文及正式规则贯穿各阶段。已有原始材料的宿主可使用`respond`完成相同流程，无须重复检索。

任务工作状态由调用方保存和恢复，不自动成为长期资产；只有显式沉淀才形成用户资产。信息使用独立于内核，运行于智能体一侧，真实智能体与模拟 Reader 复用[同一接入实现](../modules/information-use/README.md)，不内置模型或供应商调用。

### 外部智能体如何接入信息使用组件

**通用的是信息交接与结果契约，实际调用由智能体宿主完成；连接信息内核不等于自动装配信息使用能力。**

| 部分 | 负责什么 |
| --- | --- |
| 通用组件 | 提供初始线索准备、交付指令与结构、最终结果组合；不绑定模型、供应商或智能体。 |
| 接入适配 | 在宿主侧装配组件，交付原始需求和原文，沿用权限、模型、档位、凭据管理及恢复机制，校验结构化结果。 |
| 外部智能体及其运行环境 | 主导用户任务，自主继续取证，理解原文、判断依据与条件并完成需求。 |

已有工具循环的宿主先按`FRAME`与`FRAME_SCHEMA`独立明确任务依据，用`task_contract(frame)`生成交付指令和结构，再以`initial_context(query, call)`准备线索、继续自主取证，最后调用`finish(response)`呈现结果。只处理已获材料时，`respond(observations, invoke)`负责相同流程；`invoke(stage, instruction, payload, schema)`由宿主实现，不隐式继承其他任务或历史答案。组件不需要知道模型来自轻量客户端、Codex还是其他运行环境。

中断后沿用宿主的工作记录与恢复机制；未完成推理是可纠错工作，不是原始证据。`stage_prompt`可携带同一步骤的工作笔记，格式修复应保留当前已获材料，不因无损参数编码或交付格式问题重新获取全部信息。

以Codex为例，适配可通过App Server向其对话交付需求、材料、工具清单及结果结构，再接回结果，无须修改Codex内部实现。Python接入同时支持轻量客户端和OpenCode；不同宿主仍需验证实际接入质量，不能把某一模型的成绩外推为所有宿主的效果。
...
10. 开发与验收中的外部智能调用同样服从窄腰：业务流程只声明角色、输入、结构化输出、工具能力和资源边界；供应商进程、认证定位、传输协议及事件格式止于适配器。
11. 外部智能请求与证据必须同时绑定稳定调用契约、供应商、模型、推理档位、执行制品、工具清单、超时/重试策略和并发隔离；抽象边界不得抹去可复核来源，也不得把认证内容写入身份或证据。
12. 更换外部智能实现只允许新增或替换一个适配器及其显式装配、配置与资格证据，不得修改生成、语义组织、Reader、Judge、评分或恢复等业务编排；无法保持这些业务合同的实现不能被视为等价替换。
...
连接位置、体系身份和接入主体分别表达，普通使用与所有者管理保持独立。具体使用行为见[跨地点访问与便捷迁移](../product/cross-location-access.md)，接入保护、任务接续与唯一权威交接见[接入与迁移架构](../modules/access/README.md)；实施状态由对应工作包维护。
