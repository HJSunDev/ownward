...
信息使用层围绕原始任务、带来源说明的观察材料和接入智能体的工作产物提供协作支撑；原文、任务要求、模型推断与计算结果不得互相冒充。程序负责材料与过程的可靠衔接，外部智能体负责理解、适用范围、冲突解释及结果取舍。它不替调用方认证来源真实性，结构检查通过也不等于任务质量已经提升。具体如何分配工作、安排上下文与反馈，由真实任务中的质量和成本证据决定，不强制所有任务采用工作区、判断图或固定流程。

任务工作状态属于可丢弃、可由调用方保存和恢复的运行状态，不自动进入长期资产；只有显式的信息沉淀操作才能形成用户资产。信息使用是独立契约和独立组件，通过产品装配与接入层暴露，改动不改变信息内核身份。真实智能体和模拟 Reader 通过同一产品能力使用它；模拟流程中的模型调用属于外部智能替身，不能迁入产品。候选实现见[《信息使用能力》](../modules/information-use/README.md)；未证明收益的实验不作为已交付能力。
