...
flowchart TB
    ...
    IU["信息使用层<br/>任务依据 · 判断依赖 · 核验反馈 · 局部修正"]
...
**信息使用层通过代码编排多次独立上下文的模型调用，每次只交付当前步骤的指令与所需材料；理解、推理和判断由接入方自己的智能完成。** 涉及事实判断与修订的步骤保留原始需求与全部已获原文；只决定下一步安排的调用可以使用已有工作状态，但不据此建立事实。中间解释和方案不能冒充原始证据，代码不判断开放语义。

当前处理流程在获取信息后运行：

1. **理解与初稿：** 无现成初稿时并行调用两次，一次理解原文表达，一次尝试完成用户任务，两次上下文独立；已有初稿则直接复用，只调用理解步骤。
2. **竞争方案：** 将原文、理解结果与初稿交给一次新调用，为同一任务提出有证据支持的另一方案；不为求异编造事实，没有成立的替代方案则保留初稿。下一步比较两份结果。
3. **比较修正：** 将原文和两份方案交给一次新调用，依据原始需求比较、选择并提出必要的精确修改；代码执行明确修改。若修改无法定位，交回同一外部智能核对工作文本与既定修改，仅修正定位；修改内容不变，仍无法唯一应用则明确失败，不静默丢弃或猜测替换。
4. **查漏补缺：** 将原文、选定结果和两份方案交给一次新调用，找回尚未交付的有用结果，不重写已有结果。
5. **独立核验补充：** 仅在有补充提案时调用；只交付原始需求、原文和补充提案，避免继承旧方案的判断。外部智能确认成立才由代码追加，否则丢弃。
6. **决定是否检查结论依赖：** 用一次独立调用比较原需求、已完成工作和已有判断依据，不重读材料或重新作答。若结论仍依赖一个已知缺失、又未妥善限定的事实关联，才进入下一步；正常推断和已有条件说明不自动触发全面复核。
7. **按需核验依赖：** 将原需求、全部已获原文和已完成工作交给独立调用；保持已表达事实不变，去掉未经证实的关联，检查哪些结果仍成立、哪些必须附带条件。仅应用必要的精确修订，保留已有有效内容；选择理由不交给核验者充当证据。

**默认入口让外部智能在现有响应中给出当前工作与是否使用组件的选择；直接完成的选择由同一外部智能在独立上下文中核对任务需求。** 核对接收原任务、已获材料与暂定结果，只决定处理方式、不重新作答；暂定结果用于暴露取舍，不作为事实依据，依据实质解释或取舍需求按需启动。选择组件则复用初稿，从理解步骤接续；原响应主动选择协作为6次调用，有补充为7次，启动依赖核验再增加1次。原响应选择直接完成则额外增加1次核对，确认直接完成仍共2次。这些次数不含此前检索过程及异常恢复；调用方只依赖契约，不依赖内部步骤数量，所有判断仍由外部智能承担。

任务工作状态属于可丢弃、可由调用方保存和恢复的运行状态，不自动进入长期资产；只有显式的信息沉淀操作才能形成用户资产。信息使用是独立契约和独立组件，运行于接入智能体一侧，改动不改变信息内核身份。真实智能体和模拟 Reader 复用同一实现，模型调用由接入方提供，不能迁入信息内核；接口与接入方式见[《信息使用能力》](../modules/information-use/README.md)。

### 外部智能体如何接入信息使用组件

**通用的是协作流程与调用契约，不是对任意智能体的自动适配。** 信息内核提供信息操作，规则说明使用约定；信息使用组件提供处理流程，接入适配把流程要求转换成外部智能体运行环境支持的调用。外部智能体仍主导用户任务，Ownward 不接管它的内部实现，也不内置替它思考的模型。

| 部分 | 负责什么 |
| --- | --- |
| 通用组件 | 决定下一步交付哪些指令、原文和工作产物，接收结构化结果并衔接后续步骤；不绑定模型、供应商或智能体。 |
| 接入适配 | 在智能体一侧装配组件，将能力暴露为它可调用的工具或本地接口；利用宿主提供的接口安排独立上下文、提交步骤材料并返回结果，沿用接入方的权限、模型和凭据管理。 |
| 外部智能体及其运行环境 | 按需使用协作能力，提供实际推理与上下文执行能力，完成各步骤的理解、判断和结果交付；组件代码不代替这些判断。 |

实际链路是：**外部智能体调用协作入口 → 通用组件交付当前步骤 → 接入适配调用宿主的独立上下文 → 外部智能完成该步骤 → 结果返回组件并最终交回任务主角。** 自动接入的触发核对也通过同一适配执行，不另设内部智能。当前 Python 实现由接入方提供 `invoke(stage, instruction, payload, schema)` 回调；流程只调用这个契约，不知道背后是轻量智能体还是其他宿主。

“独立上下文”指每次只接收该步骤应有的材料，不隐式继承其他步骤的对话与判断；可由宿主的新对话或具有同等隔离能力的调用实现，不意味着换模型或新建另一个产品。轻量智能体可用独立消息上下文调用自己的模型服务；其他智能体需通过各自支持的接口完成同样的交接。因此更换接入方应更换适配，不应重写通用处理流程。

同一步骤中断后，接入方可保留未完成工作，在重试时与原指令、原材料一并交回自身智能继续处理；工作笔记可纠错，不是原始证据，也不能跨步骤或跨任务继承。适配层转换错误类型时必须保留这些工作，不能只传递错误消息而丢失进度。Python组件提供`stage_prompt`组装这种续接材料，实际中断检测、重试和模型调用仍由接入方负责。

**接入前提必须验证：宿主允许工具或本地接口调用，并能提供所需的独立上下文、多次推理和结果返回能力。** 仅连接信息内核的 MCP 不会自动运行客户端组件；不具备这些条件的接入方，不能承诺获得同等流程能力。能接入、按需触发有效、完整处理有效是三个不同的验证结论。

以 Codex 为例，适配可通过其 App Server 接口创建独立对话、提交材料并接回结果，而非修改 Codex 内部逻辑。2026-09-08 的最小实测以两条会议室变更记录为输入，由 Codex 调用协作工具，进入现有组件首阶段，再在独立 Codex 对话中完成原文理解并返回结果；已确认两个对话相互独立，主对话的额外内容未传入处理步骤。这只证明该接入路径可行，不代表桌面端仅连接MCP即可自动获得组件，也不代表完整流程和触发机制已在Codex上验收。
