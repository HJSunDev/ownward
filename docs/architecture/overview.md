...
物主视图端口 `ownward.owner-view/v1` 与 MCP 接入平级，薄投影直接读取同一权威与派生契约；本机入口复用原回环监听与物主验证，不新增智能、常驻进程或业务存储。标准 MCP 的限域草稿工具与浏览器条件提交共用工作状态契约；恢复、迁移和双轨批准沿原权威流程。五面网页仍由物主窗口下一执行单元交付。
