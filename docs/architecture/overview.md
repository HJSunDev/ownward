...
4. **形成结果：** 外部智能逐项明确已有依据、未决关系及必要条件，再形成主结果与有据的条件结果。
5. **核对并交付：** 独立外部上下文核对主结果与所报依据的依赖关系，有实质不符才返回完整修订正文；代码保留未修改的主结果，或使用修订正文，再原样附上条件结果，不推断事实。

标准流程包含任务准备、取证并形成结果、依赖核对三个外部智能阶段，取证阶段可多轮调用工具；原任务的必要上下文及正式规则贯穿各阶段。“一次交付”指用户最终得到一份结果。已有原始材料的宿主可使用`respond`完成相同流程，无须重复检索。
...
已有工具循环的宿主先按`FRAME`与`FRAME_SCHEMA`独立明确任务依据，用`task_contract(frame)`生成交付指令和结构，再以`initial_context(query, call)`准备线索、继续自主取证，最后调用`review_delivery`核对并交付。只处理已获材料时，`respond(observations, invoke)`负责相同流程；`invoke(stage, instruction, payload, schema)`由宿主实现，不隐式继承其他任务或历史答案。组件不需要知道模型来自轻量客户端、Codex还是其他运行环境。
