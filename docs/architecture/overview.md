...
**信息使用层通过代码编排多次独立上下文的模型调用，每次只交付当前步骤的指令与所需材料；理解、推理和判断由接入方自己的智能完成。** 各步骤均保留原始需求与全部已获原文，中间解释和方案不能冒充原始证据，代码不判断开放语义。
...
3. **比较修正：** 将原文和两份方案交给一次新调用，依据原始需求比较、选择并提出必要的精确修改；代码执行明确修改。
4. **查漏补缺：** 将原文、选定结果和两份方案交给一次新调用，找回尚未交付的有用结果，不重写已有结果。
5. **独立核验补充：** 仅在有补充提案时调用；只交付原始需求、原文和补充提案，避免继承旧方案的判断。外部智能确认成立才由代码追加，否则丢弃。

**默认入口让外部智能在现有响应中给出当前工作与是否使用组件的选择；直接完成的选择由同一外部智能在独立上下文中核对任务需求。** 核对接收原任务、已获材料与暂定结果，只决定处理方式、不重新作答；暂定结果用于暴露取舍，不作为事实依据，依据实质解释或取舍需求按需启动。选择组件则复用初稿，从理解步骤接续；原响应主动选择协作为5次调用，有补充为6次，原响应选择直接完成则额外增加1次核对，确认直接完成共2次。这些次数不含此前检索过程；调用方只依赖契约，不依赖内部步骤数量，所有判断仍由外部智能承担。
...
同一步骤中断后，接入方可保留未完成工作，在重试时与原指令、原材料一并交回自身智能继续处理；工作笔记可纠错，不是原始证据，也不能跨步骤或跨任务继承。Python组件提供`stage_prompt`组装这种续接材料，实际中断检测、重试和模型调用仍由接入方负责。
