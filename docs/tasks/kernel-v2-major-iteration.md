...
状态：阶段 3 已以不含盲测内容的聚合收据完成 15 题墙钟归因并重新关闭；Stage 4/5 候选及内部检查点按直接依赖机械复用，当前已重新达到从全新 5 题启动阶段 6 的标准，但尚未生成新关卡。旧计划 `c74ed563…52f5` 的 V2/V0 质量均为 15/15，`833.279>751 s` 的唯一失败已证明来自 Stage 6 控制器把串行、逐调用冷启动的共享生成/准入和 V0 对照墙钟错误归为候选失败，而不是 V2 内核。修正后生成使用 4 个独立单-turn lane、按冻结原序重组，准入仍在全部生成完成后执行；保守只承认两路有效并发并计入 `16.709 s` 重复误差，整级投影为 `568.401 s`，距原 `751 s` 门保留 `182.599 s`。V0 是唯一有效评测基线；当前产品仍保持 V1 行为，V1 未晋升。V2 subject `834e204c…f9f17c`、内核世代、四个内部检查点、三条基线历史和正式 state 均未改变。旧 15 题内容已销毁且禁止重跑；25/50、正式 LongMemEval-S 与产品切换尚未开始。
...
| 2. 验证能力补齐 | 已完成 | 版本化验证入口与非候选校准报告 | 先验准入在原始内容销毁前持久化覆盖、检查和组合维度的不可逆聚合；诊断计划 `9c2d1af9…522913` 将首根因定位为 temporal-order 的难度、表面捷径与评分区分力共同失败。稳定会话引用、时间双跳、多干扰双跳与单批持久 Codex 通道修复后，两批独立 15 题计划 `24e70a5c…c941ac` 共 30/30 通过，墙钟 `409.974/426.684 s`，零候选/V0 执行；原始内容已销毁且终态可零执行复用。共享直接依赖变化已精确使旧 5/15 关卡降为诊断，Stage 3—5 不受影响 |
| 3. 端到端归因与材料冻结 | 已完成（墙钟问题链重新关闭） | 问题池、开发/回归清单、五方向决策 | 收据 `72d6f045…aaef` 只读闭合 `833.279 s`：生成 `474.914`、准入 `35.667`、候选完整消费者 `135.312`、V0 `149.828`、控制器/启动/恢复余量 `37.557 s`。独立且材料已销毁的两批 15 题资格为 `409.974/426.684 s`，证明首个可控根因是 Stage 6 串行逐调用 App Server 调度；候选质量与内核不是失败责任方 |
...
| 6. 一次性盲测 | 待从全新 5 题重启 | 不含原始题面的四级累计报告 | 旧 15 题 `c74ed563…52f5` 已按收据重分类为评测流程失败而非候选失败；其内容已销毁且不得重跑。Stage 6 v3 控制器与四级合同已重新封存，全部旧关卡因控制器身份变化失效；下一步只能生成全新 5 题，25/50 未开始 |
...
阶段 6 由唯一控制器和统一 CLI 顺序承载 5/15/25/50 四级关卡；修正后四份结果前冻结的 v3 合同 `iteration/v2/stage6-blind-gate-{05,15,25,50}-contract.json` 身份依次为 `1a59fde3…e9d9`、`fb5939fb…d941`、`9b2eb002…fce8`、`8ae9dc99…8dd8`。每份合同保持原题量、覆盖、预算、绝对/相对门和下一动作，只新增 4 个独立单-turn 生成 lane、冻结原序重组、“全部生成后才准入”及“评测流程超时不冒充候选失败”的边界；整级效率门仍强制执行。历史收据保留全部旧诊断、恢复事故计划、最后通过的旧 5 题和被重分类的旧 15 题；所有旧关卡均不适用于当前控制器，V0/V1 与正式内部验收未被改写。
...
阶段 2 可靠性合同 `iteration/v2/stage2-blind-admission-reliability-contract.json`（身份 `fe2bd18d…bdaa`）在诊断前冻结 15 题五类各 3 题、两批资格、单批 `492 s` 和总计 `984 s` 门。诊断计划 `9c2d1af9…522913` / 结果 `2e1f794e…edeb95` 在销毁原始内容前留下不可逆聚合：唯一拒绝来自 temporal-order，`difficulty_sufficient`、`no_surface_shortcut`、`scoring_discriminative` 同时失败。实现以稳定会话枚举消除引用错绑，以隐藏连接键把值证据与时间/多干扰选择证据机械分离，并拒绝任何第二条完整路径；每批 16 次调用顺序复用一个单-turn App Server，未改变模型、检查或角色隔离。

最终非候选资格计划 `24e70a5c…c941ac` / 结果 `8df3fee5…99adb6` 的两批相互独立材料均为 15/15，墙钟 `409.974/426.684 s`，总计 `836.658 s`；生成/准入调用 `30/2`，仅第一批有 1 次合同内结构重试，零限流，候选与 V0 执行均为 0。两批原始内容均已销毁；全新进程仅凭 plan identity 终态复用为零模型、零产品执行。预算迁移收据内置于 `blind-calibration-budget.json`（身份 `a9305977…40619`），保留原 `406/751/1097/1961 s` 正常预算和 `320/492/665/1097 s` 失败预算，未放宽任何门。Stage 4 原始向量生命周期的精确依赖迁移收据 `stage4-resource-cost-raw-vector-lifecycle-dependency-migration.json`（身份 `6ea61581…ce57`）只承认本次 Stage 2 CLI 加法式分发和收据校验器自身变化；Stage 4 合同、源码、门槛、正式状态摘要与既有证据保持原样，任何未列出漂移仍失败关闭。终态 Acceptance Suite `302/302`、LongMemEval-S 适配器 `41/41`、`check`、`self-check` 与资格 plan identity 零执行恢复均通过，正式 state 仍为 `0587d7f9…da01`；唯一下一动作是用修正后的共享依赖从全新 5 题重启阶段 6。
...
盲测正式启用前的历史非候选五题校准冻结了 5/15/25/50 题正常预算 `406/751/1097/1961` 秒、失败预算 `320/492/665/1097` 秒，正常总计 `4215` 秒，低于 `9000` 秒设计上限；其原始内容已销毁且不参与候选判断。共享生成—准入身份修正后，新的两批 15 题非候选资格以 `409.974/426.684 s` 和 `836.658/984 s` 重新证明这些预算无需放宽。预算文件保留旧校准为历史来源，并以当前资格计划、结果及直接依赖形成可机械校验的迁移收据。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 阶段 3 墙钟归因与 Stage 6 控制器修复已完成；Stage 4 候选和 Stage 5 四层检查点直接依赖未变并已机械复用。当前停在重新生成全新 5 题之前 |
...
| 盲测级别 | 当前控制器下没有有效关卡；旧 5 题 `82cfab1b…c2b9` 与旧 15 题 `c74ed563…52f5` 均只保留历史聚合，后者是评测流程失败而非候选失败。旧恢复事故 `ac540d14…3b6c` 不计分；25/50 未开始 |
| 当前证据 | Stage 4 表示生命周期合同/可行性/开发/回归/最终 AB/BA 为 `e39272da…93ce8` / `fa20e1bf…fd38` / `c1609a82…cbb3` / `afc337b7…9215` / `3e06bfd3…fff72`；Stage 5 冻结/预检及 frontier/core/qualification/full 为 `8a08485a…c839` / `84708c12…2418` / `f04af6b1…697a` / `4d4af8db…2881` / `dcc0eebe…37e1` / `976a1eb8…04b6`；Stage 2 资格为 `8df3fee5…99adb6`；Stage 3 墙钟归因合同/结果为 `0015fcc6…23b1` / `72d6f045…aaef`，Stage 6 历史为 `064b70ef…cdf8`。正式 state 未写，四层内部检查点与三条历史保持不变 |
| 下一验证点 | 使用 v3 控制器从全新且不重合的 5 题重新启动 Stage 6；不得恢复或原样重跑任何旧 15 题、越级进入 15/25/50、阶段 7 或正式 LongMemEval-S |
