...
状态：评测纠偏和外部智能切换收尾已完成。V0/V1/V2 主动检索同尺非盲比较均为 `20/20`；当前 V2 候选在同质量下具有最低墙钟、检索均值、P95 与数据量，并已完成 Stage 3/4 定向重闭合。95 题版本套件及准入证书保持封存。提交 `67fdc76` 后错误地提前启动了最终内部验收：资格集已通过，完整专项集形成 10/24 个可恢复检查点；这些证据保留但暂停，当前候选应先从 5 题开始完成四级盲测，全部通过后再恢复剩余内部验收。
...
| 5. 候选冻结与盲测准入 | 已完成（当前候选） | 干净候选身份、完整回归与低成本内部保护 | 提交 `67fdc76` 的独立 V2 制品由通用 Stage 4 闭合汇总 `bcd3c1df…eedd8` 与候选清单 `fd5f2127…1ddaf` 证明质量、性能、资源和恢复门槛；不要求先完成 qualification/full 的外部智能准备或执行 |
| 6. 一次性盲测 | 当前候选等待 5 题 | 不含原始题面的四级累计报告 | 套件 `346852e3…ed5` 与先验准入保持封存；为当前候选创建显式 evaluation batch，按 5/15/25/50 依次评价。旧候选的 5/15/25 结果保持历史不可改写，不授权当前候选跳级 |
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 提交 `67fdc76` 的 Stage 3/4 定向重闭合已完成，满足低成本盲测准入。提前运行的资格集已通过，完整专项集形成 10/24 个检查点后暂停；当前唯一下一阶段是该候选的 5/15/25/50，四级通过前不继续完整专项集 |
...
| 正式状态 | 提前执行后唯一 state SHA-256 为 `99a416dc…c40d`，保留 frontier/core/qualification 检查点和三条 V0 基线历史；隔离 full 目录保留 10 个 sealed result，但没有 full 顶层报告。上述现场不构成顺序合法的最终内部验收，本轮不改写、不删除；Stage 6 与 community 均未执行 |
...
| 方向状态 | Stage 3 交付根因已关闭；Stage 4 当前候选开发 4/4、固定回归 8/8，38—62 资产四 worker 的 80 次零模型复核 p95 `506.432 ms <= 553 ms`，选择与必要来源交付稳定。表示、语义和存储成本通过直接依赖迁移继续有效；Stage 5 已以通用候选清单和通用 Stage 4 闭合汇总完成冻结，下一步只能进入四级盲测 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效；旧 batch 的 5/15/25 结果只绑定旧候选并保持不可改写，不能作为 subject `460e3f82…b5fd` 的关卡结果；当前候选尚未开始 5 题 |
| 当前证据 | 修复终态 `.tmp/kernel-v2-major-iteration/stage6-context-delivery-repair-v2/answer-sufficiency/ae4d94fe…78bb/result.json` 身份 `bd08c8c5…e5a1`，3/3 diagnosis、2/2 confirmation、4/4 regression 全过，Reader/Judge 接受 30/30 且逐字重放成立；开发/回归结果身份 `b68d0359…0ed7` / `0463da09…c125`；当前候选充分样本时延结果身份 `d65938d6…c950`。Stage 5 合同 `2ce8bd01…5ac12` 只消费通用闭合汇总，重新冻结结果身份 `3052d44e…d029`；qualification/full 入口现强制校验同一候选的完整 5/15/25/50 身份链，V0 分区结果按候选无关直接依赖寻址。同身份恢复均为零模型、零产品执行；本轮正式 state 保持 `99a416dc…c40d` |
| 下一验证点 | 协调者先提交本轮评测执行修复；随后为提交 `67fdc76` 创建 evaluation batch，按同一封存套题运行 5/15/25/50。四级全部通过后，使用同一候选恢复 qualification 既有结果和 full 的 10 个 sealed result，仅补齐真实失效或缺失的 14 项并完成交接预检 |
