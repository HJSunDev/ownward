...
状态：阶段 2 进行中。V0 是唯一有效评测基线；当前产品运行的是迁移后保持 V1 行为的组合，V1 未晋升；V2 候选尚未建立。阶段 1 的比较合同已在接触任何 V2 候选结果前冻结。
...
| 迭代能力 | `benchmarks/acceptance/suite/kernel_iteration_run.py` 是唯一通用非正式入口；`iteration/v2/comparison-contract.json` 冻结 V0、当前产品、V2 subject 合同、三维门槛、证据类型、失效与成本边界。原子计划/检查点可恢复且不能写正式 state；V1 专用入口已退出生产路径 | 阶段 2 只在该入口上补齐开发/回归执行器、独立盲测生成与准入，并先完成不参与候选判断的五题校准 |
...
| 2. 验证能力补齐 | 进行中 | 版本化验证入口与非候选校准报告 | 通用 subject、证据类型、直接依赖、原子检查点和恢复骨架已成立，并证明不进入正式执行模式、证据层或 `state.json`；尚需补齐开发/回归实际执行、盲测生成与先验准入，并完成不参与候选判断的盲测 5 题校准 |
| 3. 端到端归因与材料冻结 | 待开始 | 问题池、开发/回归清单、五方向决策 | 正式聚合缺陷已抽象；独立开发案例复现首个根因，代表性回归保护 V0 正确能力；V1 的已知收益均已完成同尺重证与可分离性裁决，成立者进入保护项，不成立者留下拒绝证据；五个方向均完成审视并冻结本轮负责方向、目标、保护项和下一验证点 |
...
盲测正式启用前，必须用不接触候选判定的 5 题完成一次真实校准，冻结质量准入、并发、重复误差、单级预算和总预算。现有 `132.359` 秒 / 4 题预检只支持“候选与基线顺序运行 95 题约 104.8 分钟、含生成与准入的四级正常路径上限 150 分钟”的保守设计模型，不得冒充盲测实测；校准失败先修复执行路径，不得降低质量或静默放宽标准。
...
| 盲测级别 | 未开始；一次性盲测入口尚未实现 |
| 当前证据 | 干净检出可校验的比较合同与 V0 聚合事实；`.tmp/kernel-v2-major-iteration/` 中的 V0/当前产品身份校准、显式 v6/v3 运行态校准、原子恢复与正式 state 不变证明；V2 合成 subject、直接依赖及可选运行态依赖的局部失效和同尺配对仅由隔离测试证明，尚无 V2 候选结果 |
| 下一验证点 | 只沿通用入口补齐独立开发/回归执行与盲测生成、先验质量准入，随后用不参与候选判断的 5 题完成职责隔离、销毁、重复误差和预算校准 |
