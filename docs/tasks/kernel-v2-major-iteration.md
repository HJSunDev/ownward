...
状态：阶段 2 已完成，阶段 3 进行中。V0 是唯一有效评测基线；当前产品运行的是迁移后保持 V1 行为的组合，V1 未晋升；V2 候选尚未建立。比较合同、通用非正式执行闭环和一次性盲测机制均已在接触任何 V2 候选结果前冻结并完成新控制器身份下的非候选五题校准。
...
| 迭代能力 | `benchmarks/acceptance/suite/kernel_iteration_run.py` 是唯一通用非正式入口；`iteration/v2/comparison-contract.json` 冻结比较政策，`iteration/v2/validation-contract.json` 冻结开发/回归/整体验证与盲测职责，`iteration/v2/blind-calibration-budget.json` 冻结实测预算。原子计划/检查点可恢复且不能写正式 state；V1 专用入口已退出生产路径 | 阶段 3 可直接用同一入口建立问题池、开发与代表性回归材料；尚未建立 V2 候选或候选结论 |
...
| 3. 端到端归因与材料冻结 | 进行中 | 问题池、开发/回归清单、五方向决策 | 正式聚合缺陷已抽象；独立开发案例复现首个根因，代表性回归保护 V0 正确能力；V1 的已知收益均已完成同尺重证与可分离性裁决，成立者进入保护项，不成立者留下拒绝证据；五个方向均完成审视并冻结本轮负责方向、目标、保护项和下一验证点 |
...
| 方向               | V1 已知观察                                                                  | 初始状态 |
| ------------------ | ---------------------------------------------------------------------------- | -------- |
| 信息表示与组织结构 | 细粒度证据降低上下文规模，但完整片段、更新关系和冲突事实交付不足             | 待审视   |
| 检索架构与算法     | 搜索未返回目标证据由 116 降至 6；读取后仍答错由 9 增至 140，查询时延显著退化 | 待审视   |
| 语义能力与表示模型 | 长资产语义召回改善，尚未证明最终证据充分性                                   | 待审视   |
| 数据结构与存储架构 | 数据规模和存储收敛真实改善；默认保护，除非新证据证明仍是瓶颈                 | 待审视   |
| 执行架构与状态维护 | 创建与耐久成本改善，查询链路成本退化；需拆分直接原因                         | 待审视   |
...
| V2 候选 | 未建立 |
| 当前根因 | 未确认；V1 事故只提供表象与待证明假设 |
| 方向状态 | 五个方向均待审视 |
| 保护边界 | V0 已有正确能力；V1 已知收益待阶段 3 同尺重证与可分离性裁决 |
| 冻结门槛 | 三个维度分别冻结大幅跃升与不可退化门槛，禁止跨维度补偿；V0 高基线能力逐项保护，V1 收益仍待同尺重证 |
| 盲测级别 | 候选盲测未开始；独立生成、准入、执行、评价、销毁、恢复机制及非候选五题校准已成立，校准数据不得用于后续候选判断 |
| 当前证据 | 比较合同与验证合同可在干净检出校验；预算冻结身份 `90d2b744…a224` 绑定计划 `7db58bb3…18ce`、控制器 `548fc83d…4577`、校准结果 `e05ea3bb…4689` 与当前依赖校验器 `059bda21…2519`；本机当前定位收据为 `4411901a…d931`，其路径身份不进入可移植的版本化预算。独立进程仅凭 plan identity 在全部当前直接依赖机械一致时返回 `model_calls=0`、`product_executions=0`；历史终态仍可审计读取，但任一执行器、环境、二进制、向量、模型或运行态校准漂移都会拒绝作为当前预算/恢复证据。正式 state 摘要不变。`.tmp/kernel-v2-major-iteration/` 只保存运行态只读校准及不可逆五题聚合证据；原始题面、真值、证据和逐题输出已销毁，尚无 V2 候选结果 |
| 下一验证点 | 阶段 3 只从正式聚合缺陷抽象问题池，以独立非重合材料复现首个端到端根因，并冻结代表性开发/回归清单及五方向决策；不得复用校准数据或开始候选优化 |
