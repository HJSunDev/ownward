...
状态：当前 subject `a0a59771…117` 的 Stage 4 与干净 HEAD `a74a2e1…65e9` 封存的 Stage 5 制品继续有效；frontier、core、qualification、full 四层同身份恢复均为零执行复用。Stage 6 已迁移到“每个内核大版本先一次性准备并封存一套完整 5/15/25/50 题套件、后续只按分区评价显式 candidate/baseline 批次”的通用路径。V2 套件 `346852e3…ed5` 已完成 95/95 槽位及逐槽准入证书封存，候选和基线执行均为 0；旧四级逐关生成与评价结果只保留历史诊断，不参与新套件的候选判定。当前唯一下一动作是为既有冻结候选与明确基线建立 evaluation batch，并从该套件的 5 题分区开始，不运行正式 LongMemEval-S。
...
| 6. 一次性盲测 | 完整版本套件已封存；候选评价未开始 | 不含原始题面的四级累计报告 | 通用版本套件合同 `33a11353…00e` 已一次性封存 V2 的 95 题与 95 份逐槽准入证书，分区为 5/15/25/50；套件身份 `346852e3…ed5`，公开收据 `2200e3fa…288`。准备阶段只局部替换首轮拒绝的 11 槽，84 个已接受槽位没有复审；正式 state 未写，candidate/baseline 执行均为 0。旧逐关生成结果仅作历史诊断，不得与新套件混用 |
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | Stage 4 与 Stage 5 继续有效，full 为 24/24。Stage 6 的 V2 完整 95 题版本套件已一次性封存，新的 candidate/baseline 评价尚未开始；正式 community 继续禁止 |
...
| 当前根因 | 旧逐关控制器把材料生成、准入与候选评价耦合，导致合格材料重复生成、历史关卡无法组成一个版本级冻结泛化边界；该执行架构根因已由一次准备、逐槽证书和显式 evaluation batch 关闭。旧 25 题聚合失败及定向非复现裁决继续只作历史诊断，不能直接归因给当前内核 |
| 方向状态 | 最终回答充分性问题链已在正确的 access/消费者层关闭；内核效果身份未变。Stage 4 当前候选重证和 Stage 5 内部整体验证均已完成；Stage 6 只完成版本套件准备，尚未产生新套件下的候选结论，不提前关闭大版本 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 含唯一冻结的 5/15/25/50 分区，身份依次为 `0c31d4c0…836f` / `6beed6d0…7359` / `0a341a9e…3e59` / `4f17a743…76`；95 个槽位均有逐槽机械证明和语义准入证书。尚未运行任何新套件分区，旧逐关结果只作历史诊断 |
| 当前证据 | Stage 4 迁移 `fd82621e…998`、开发 `e29d2b3f…d6a` 4/4、回归 `22a606b2…cde` 8/8；Stage 5 freeze `e43e5666…599`，frontier/core/qualification/full 报告 `ed4d35c1…d886` / `be6c60eb…40deb` / `0813929d…62b2` / `4a1c4d98…24fd`，full 24/24；版本套件 plan `c875143c…53d7`、suite `346852e3…ed5`、公开收据 `2200e3fa…288` 与准入资格 `45345923…3372`。所有终态按 plan identity 零执行复用，正式 state 未写入 |
| 下一验证点 | 只为当前冻结候选与明确基线生成显式 evaluation batch，验证其身份、直接依赖和 freeze receipt 后运行套件的 5 题分区；不得重新生成已接受槽位、改写版本套件、进入更高级分区、community preflight 或正式 LongMemEval-S |
