...
状态：待开始。V0 是唯一有效评测基线；当前产品运行的是迁移后保持 V1 行为的组合，V1 未晋升；V2 候选尚未建立。
...
| Acceptance 状态 | 唯一 binding/state 为 v6/v3；`baseline=null`，保留三条 V0 基线历史，当前四个内部检查点绑定 V1 | 阶段 1 至 4 只读；不得靠写入 `baseline`、洗写旧检查点或重跑旧证据修复比较关系。V2 只有达到阶段 5 后才允许将验收对象重绑为 V2 并生成自身内部证据；重绑只切换当前验收对象，不切换产品当前内核，也不改写 V0/V1 的制品、状态或既有证据 |
...
| 迭代能力 | Acceptance Suite 已有身份、直接依赖失效、原子检查点和恢复；`kernel-iteration` 仍硬编码 V0 Git 来源、工作树候选及 V1 合成视图，尚无 V2 开发集、代表性回归和一次性盲测入口 | 在正式体系内补齐通用 V2 迭代证据闭环，不复制 V1 专用路径或建立第二套验收状态 |
...
| 阶段 | 状态 | 唯一交付 | 关闭条件 |
| --- | --- | --- | --- |
| 1. 比较合同与恢复基线 | 待开始 | V2 迭代合同、V0 同尺对照清单 | 从 v6/v3 状态、V0/V1 世代目录和冻结制品机械确认三类身份；分别冻结信息组织质量、检索与最终回答质量、端到端效率的指标和跃升门槛，以及共享评测条件、内核专属变量、保护边界、重复误差、成本模型和证据失效图，且未写正式 state |
| 2. 验证能力补齐 | 待开始 | 版本化验证入口与非候选校准报告 | 在 Acceptance Suite 内形成通用的问题池、开发、回归、内部整体验证及一次性盲测非正式入口、检查点和证据合同，并机械证明其不进入正式执行模式、证据层或 `state.json`；完成不参与候选判断的盲测 5 题校准，证明职责隔离、销毁、恢复和预算成立 |
...
| --- | --- |
| 当前阶段 | 阶段 1：比较合同与恢复基线 |
| 评测基线 | V0 世代 `1952f4b8…7b77`、映射内核效果 `61729fb8…017a`；同尺比较身份待阶段 1 冻结 |
| 当前产品 | 组合 `c068ae20…751e`、内核 `db18a3a1…2684`；保持 V1 行为，不是有效评测基线 |
| 正式状态 | Acceptance binding/state v6/v3；`baseline=null`，三条 V0 历史存在，内部检查点绑定 V1；阶段 5 前只读，阶段 5 后只允许将验收对象重绑到冻结 V2 并记录其内部证据，不改变产品当前内核或 V0/V1 自身状态 |
| 迭代合同 | 未建立 |
| V2 候选 | 未建立 |
| 当前根因 | 未确认；V1 事故只提供表象与待证明假设 |
| 方向状态 | 五个方向均待审视 |
| 保护边界 | V0 已有正确能力；V1 已知收益待阶段 3 同尺重证与可分离性裁决 |
| 冻结门槛 | 未建立；必须在接触 V2 结果前由 V0 同尺测量与方法要求确定 |
| 盲测级别 | 未开始；一次性盲测入口尚未实现 |
| 当前证据 | V0/V1 版本档案、V1 事故报告、内核世代目录、v6/v3 Acceptance 状态和现有 V1 专用迭代入口 |
| 下一验证点 | 建立不改正式 state 的 V0 同尺比较合同，明确复用能力与必须替换的 V1 专用验证路径 |
