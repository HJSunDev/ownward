...
状态（2026-09-05）：冻结 V2 `460e3f82…b5fd` 已完成 5/15/25 题及 8 题风险补充验证；50 题原始成绩 45/50，确认两道题目缺陷与三道来源组织/展示缺陷。两道题仅修正对象与说话者表述，历史、答案及其他 48 题不变，修正记录为 `iteration/v2/stage6-question-corrections.json`；原始运行不改判。评测流程已支持候选独立运行、传输故障局部恢复和原始测量离线重判；内核修复独立处理，尚未完成四级关卡或获得正式测试资格。

> 后续阶段表与执行记录属于历史检查点；与上述最新状态冲突时，以最新状态和对应机器结果为准。旧被动观察器结果仍不具产品证明力。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | V2 候选保持冻结；最新主动检索 5 题候选执行按现行关卡政策离线重判通过，下一步直接运行 15 题。资格集与 full 10/24 现场继续暂停 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效。当前 batch `d4bcfca1…8f87b` 的最新 5 题候选为 `5/5`，已按现行绝对质量门通过；15/25/50 尚未完成 |
| 当前证据 | 最新 5 题 plan/result 为 `2a0f4f3a…0ee6` / `b33ef6db…297b`；候选报告 `9f4687e0…0366` 与检查点 `825f7f1e…cddb` 是不可改写的原始测量。现行控制器只读该聚合完成离线重判，不新增模型或产品执行 |
| 下一验证点 | 直接运行 15 题；不得重跑 5 题、运行同题 V0，或因历史 `553 ms` 门再次停下 |
