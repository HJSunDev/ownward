...
状态：Stage 6 当前 validation `059ea0ab…ccb6` 下的 5/15 题与 25 题候选裁决继续有效；25 题计划 `c9b4b189…0c5a1` / 结果 `d7071af7…129dc` 的 V2 绝对门和同材料 V0 相对门均通过。全新 50 题计划 `c45a62f1…5e90` 在第二轮以 46 个保留项和 4 个替代项完成 50/50 先验准入；候选 observation `aeb6fe93…30f` 为首答 49/50、事实交付完整、时间 0.9、冲突 1.0、检索 p95 `375 ms`。绝对门未通过后，控制器只对冻结原序第 37 项执行单题归因；产品与完整 oracle 上下文各 3/3 均失败，Judge 正反对照通过，故终态 `0fdfe34e…f468` 正确记为 `evaluation-process-error`、`candidate_decision=null`、`candidate_failure=false`，V0 未运行。总判定墙钟 `1339.815<1961 s`，可逆材料与 scratch 已销毁，仅凭 plan identity 恢复为零模型、零产品执行，正式 state 仍为 `0587d7f9…da01`。该内容及旧 50 题 `0f80dc78…f670a`、`dd4afa9b…f0050` 均禁止原样重跑。Stage 2 已把失败边界机械收敛到 Luna/xhigh Reader 的完整 oracle 语义裁决，并以全新三题非候选材料将失败后诊断 Reader 重新资格化为 Terra/high；资格 plan/result `be30d6a0…128f3` / `885192c9…1284` 为产品和 oracle 上下文零失败、Judge 正误对照各 3/3、零限流与 worker 重启，恢复零执行。产品首答仍为 Luna/xhigh，提示、renderer、Schema 与 Judge 未变。唯一下一动作是协调者审查并提交本轮评测能力修复，再从 25 题有效前级创建另一份全新 50 题；当前不得进入 Stage 7 或正式 LongMemEval-S。
...
| 2. 验证能力补齐 | 已完成（oracle Reader 已独立重新资格化） | 版本化验证入口与非候选校准报告 | validation `059ea0ab…ccb6` 的材料生成、准入、8 路局部替换及原序提交资格继续有效。首答失败只按冻结原序诊断第一题，异常仍按 Reader/Judge/官方 renderer/schema/transport 安全分类；`c45a62f1…5e90` 的已有收据把当前偏离点机械限定为 Luna/xhigh Reader 在完整 oracle 证据上的语义裁决。新合同 `fd036e2e…3e62` 只把失败后诊断 Reader 冻结为 Terra/high，产品首答 Luna/xhigh、提示、renderer、Schema 与 Judge 均不变；三题非候选资格 `be30d6a0…128f3` / `885192c9…1284` 完成 15 次 Reader、24 次 Judge，产品/oracle 零失败、Judge 正误对照各 3/3，零候选与产品执行，恢复零执行。Stage 3—5 与 5/15/25 题未失效 |
| 3. 端到端归因与材料冻结 | 已完成（Reader 可靠性与首答归因合同已闭合） | 问题池、开发/回归清单、五方向决策 | 独立诊断合同/结果 `9be7e22d…bffd` / `767654c6…0fad` 与 Reader 合同/结果 `841ab01d…5805` / `25f49541…5824` 共同证明：完整证据下 medium/high 不稳定，最低稳定配置为 Luna/xhigh；措辞变化不算失败，Terra/medium Judge 保持 5/5 正确与 5/5 错误对照。首答仍须 100%，重复只归因，Stage 4/5 不失效 |
...
| 6. 一次性盲测 | 当前 5/15/25 题候选裁决有效；三次 50 题均只作评测流程失败诊断 | 不含原始题面的四级累计报告 | 最新 50 题已耐久保存候选 observation，并以 `evaluation-process-error` 阻止把 oracle Reader 异常记成候选失败；原始内容已销毁且 V0 未运行。失败后诊断 Reader 已以独立三题资格化为 Terra/high，协调者审查并提交后仍只能创建全新 50 题计划 |
| 7. 最终测试交接 | 仅被 Stage 6 的全新 50 题候选判定阻塞 | 最终测试就绪报告 | 50 题没有形成候选判定；不得运行交接预检、正式 LongMemEval-S 或晋升 V2 |
...
阶段 6 由唯一控制器和统一 CLI 顺序承载 5/15/25/50 四级关卡；四份结果前冻结的 v3 合同身份现为 `68802a4a…4ce5`、`8d97eda0…5253`、`74004c80…664d`、`a6941af0…d4e9`。题量、覆盖、预算、100% 首答绝对门和顺序不变；新增的只是 Reader 选择与失败归因直接依赖。首答正确才继续；首答错误且证据缺失是候选失败；证据完整时，候选上下文稳定失败而 oracle 稳定才是候选上下文失败，oracle/逐字相同提示机械失稳或 Judge 对照失败则是评测流程失败并停止后续级别。输出措辞或哈希变化只记录，不作为失败。历史收据保留全部旧诊断，旧关卡均不适用于当前控制器。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | Stage 2 已把失败后诊断 Reader 从产品首答 Reader 中分离，并以三题困难非候选材料将 Terra/high 重新资格化；Stage 3—5 与 Stage 6 的 5/15/25 题候选证据继续有效。最新 50 题只形成耐久候选 observation 与 fail-closed 评估错误终态，未形成候选判定；正式 community binding/preflight 仍待 Stage 6 完成后重建 |
...
| 当前根因 | `answer-failure-attribution` 的全 50 题重复诊断与同答案 Judge 检查点碰撞均已关闭；安全异常收据可区分 Reader、Judge、官方 renderer、Schema/validation 与 transport。`c45a62f1…5e90` 进一步证明 Luna/xhigh 在完整 oracle 上 3/3 失败，而传输、Schema 与 Judge 控制正常；失败后诊断现改由已独立资格化的 Terra/high 承担，产品首答仍为 Luna/xhigh。失败的 50 题候选观察保持原样、V0 未运行且错误未归责候选；当前没有新的内核质量结论 |
...
| 盲测级别 | 当前 validation 下 5 题 `d6ae3689…f3f94` / `af5680c0…a9b4`、15 题 `bf9d6e6e…c5b1e` / `f05272ee…6f5b4` 与 25 题候选裁决 `c9b4b189…0c5a1` / `d7071af7…129dc` 有效；最新 50 题 `c45a62f1…5e90` 只有候选 observation `aeb6fe93…30f` 与评估流程错误终态 `0fdfe34e…f468`，不能计作关卡结果 |
| 当前证据 | 最新 50 题候选 49/50、事实交付完整、时间/冲突 0.9/1.0、检索 p95 `375 ms`；原单题归因只消费第 37 项，产品与 oracle 上下文各 3/3 失败而 Judge 对照通过。当前 oracle Reader 资格 plan/result `be30d6a0…128f3` / `885192c9…1284` 在三题上产品/oracle 零失败、Judge 正误各 3/3，恢复零执行；Stage 6 历史身份 `f54e829a…8293`，正式 state 仍为 `0587d7f9…da01` |
| 下一验证点 | 仅由协调者审查并提交当前 oracle Reader 评测能力修复；提交后以 25 题有效候选裁决为前级创建另一份全新 50 题。不得恢复三份旧 50 题内容、重跑 5/15/25、进入阶段 7、community preflight 或正式 LongMemEval-S |
