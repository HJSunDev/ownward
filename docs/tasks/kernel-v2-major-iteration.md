...
状态：阶段 4 已完成，阶段 5 待开始。V0 是唯一有效评测基线；当前产品仍运行迁移后保持 V1 行为的组合，V1 未晋升。最终独立 V2 subject `834e204c…f9f17c` 在既有同源片段、多来源加载和检索时延收益上，引入按当前权威 revision 封存的 pending→ready 检索表示生命周期：开发 4/4、长多事实 5/5、固定回归 8/8，产品数据为 V0 的 `0.401517`，候选可控 Token 组件继续通过 `9674<=10247.5`，完整消费者 p95 为 `141 ms`。ready revision 在派生记录耐久写入且索引更新后立即确定性释放完整原文与向量任务，256 个完成资产后活动任务表为 0。零模型 AB/BA 的候选可控关键路径为 `0.4662/0.4859 s`，加 `1 s` 重复误差后 `1.4859 s`，低于 `6.711907 s` 半数门；同身份恢复逐字复用且零模型、零产品执行。正式 state、V0/V1 与当前产品均未改变；阶段 5 整体验证、盲测和正式测试尚未开始。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 阶段 4 已完成；必要同源片段、多来源证据加载、检索时延和端到端资源成本均已关闭，阶段 5 待开始 |
...
| 方向状态 | 信息表示与组织、检索架构、数据结构与存储、执行架构问题链均已关闭；语义能力按长资产召回 1.0 保护。阶段 4 五方向证据已收口，仍须由阶段 5 同一候选整体验证 |
...
| 当前证据 | 阶段 2、3 与既有质量/检索链继续成立；表示生命周期合同/可行性/开发/回归/最终 AB/BA 为 `e39272da…93ce8` / `fa20e1bf…fd38` / `c1609a82…cbb3` / `afc337b7…9215` / `3e06bfd3…fff72`。Token 组件 `9674<=10247.5`、数据比 `0.401517`、候选可控墙钟加误差 `1.485877<=6.711907 s`；恢复逐字零执行，正式 state 仍为 `3c7826e0…fcdc` |
| 下一验证点 | 阶段 5 内部整体验证与冻结：先对同一 subject `834e204c…f9f17c` 完成完整回归和最小安全预检，再按 Acceptance 正常生命周期建立 V2 自身的 frontier/core/qualification/full 证据；不得在该阶段前运行盲测或正式 LongMemEval-S |
