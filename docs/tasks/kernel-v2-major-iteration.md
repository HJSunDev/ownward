...
状态：评测纠偏已完成。V0/V1/V2 主动检索同尺非盲比较均为 `20/20`；终态 V2 候选在同质量下具有最低墙钟、检索均值、P95 与数据量，成为后续唯一候选基线。Stage 3—5 中不依赖旧观察器的证据继续有效；Stage 6 的旧 5/15/25 题结果只保留被动评测审计意义。下一阶段从同一冻结 V2 重新执行产品忠实盲测，不运行正式 LongMemEval-S 或晋升。
