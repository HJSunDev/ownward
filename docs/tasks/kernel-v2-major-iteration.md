...
状态：评测纠偏和外部智能切换收尾已完成。V0/V1/V2 主动检索同尺非盲比较均为 `20/20`；终态 V2 候选在同质量下具有最低墙钟、检索均值、P95 与数据量，成为后续唯一候选基线。Stage 3—5 中不依赖旧观察器的证据继续有效；95 题版本套件及准入证书保持封存。当前 OpenCode Go / Qwen3.8 Flash 评测链的 5 题通过；15 题在一次 Reader 局部恢复后质量 15/15，并由既有同候选独立充分样本确认检索尾值不构成候选回归；25 题出现两项完整证据下的答案错误与充分样本检索尾值失败，但独立非重合材料没有复现候选上下文缺陷，因此 Stage 6 停止在 25 题，不运行 50 题、正式 LongMemEval-S 或晋升。

> 本文后续保留的旧 Stage 6 执行记录均为历史被动观察器记录；与上述状态冲突时，以上述纠偏结论为准。
...
| V1 候选 | 历史旧被动口径为 `285/500`；该观察器不忠实于产品 | 不是第二基线；实现保持冻结，产品级收益与退化必须在主动检索同尺下重证 |
...
- 本任务产生的开发、回归和盲测证据属于 Acceptance Suite 管理的非正式候选证据，不写正式 `state.json`、不改 V0 历史结果、不晋升任何内核。每个内核大版本在接触候选评价前只生成、准入和封存一套完整 5/15/25/50 题版本套件；盲测只能进入非正式迭代入口，不得加入正式 `contract.json` 的执行模式或证据层。正式验收禁止冻结后动态生成正式证据的边界保持不变。
...
| 2. 验证能力补齐 | 已完成 | 版本化验证入口与非候选校准报告 | 外部智能通过 `ownward.external-intelligence/v1` 统一装配；默认 OpenCode Go / Qwen3.8 Flash 的 Generator、质量准入、语义、Reader 与 Judge 已分别完成代表性资格验证，模型与档位由唯一运行选择清单冻结。业务控制器不再依赖 Codex 传输、专属 Reader 选择或旧 Judge 资格身份；供应商切换只改变所选适配器与角色的直接依赖，不改评测业务规则 |
...
| 6. 一次性盲测 | 25 题拒绝，等待独立复现 | 不含原始题面的四级累计报告 | 套件 `346852e3…ed5` 与当前 batch `ec06bed0…70fe3` 保持封存。5 题候选/V0 均 5/5；15 题质量在一次局部 Reader 恢复后为 15/15，独立 100 请求时延证据授权继续。25 题局部恢复后事实交付 25/25，但首答 23/25，首个稳定归因为候选上下文充分性；检索 P95 `2641>553 ms`。既有独立非重合最终回答材料仍为 2/2，未复现候选缺陷，故不授权改内核、重跑 5 题或进入 50 题 |
| 7. 最终测试交接 | 等待新版本套件四级评价 | 最终测试就绪报告 | 必须先以显式 evaluation batch 按 5/15/25/50 分区完成同一 candidate/baseline 的不可逆评价；在此之前不得运行交接预检、正式 LongMemEval-S 或晋升 V2 |

阶段按直接依赖推进和失效：V2 比较合同或共享评测条件变化，只重开阶段 1 的受影响部分及依赖它的 V2 比较证据，不使 V0/V1 内核或其既有证据失效；验证实现、数据质量准入或校准失败只重开阶段 2 的受影响部分，不计为候选失败；开发、回归或整体验证失败回到阶段 3 至 5。盲测候选失败只失效该 evaluation batch 的候选结果并保留通用根因，版本套件、逐槽准入证书和不依赖候选的冻结身份继续有效；后续候选必须从同一套件的 5 题分区重新开始。任何失败都不得无条件重做仍有效的前置工作。
...
Stage 6 的终态架构已改为版本级完整套件，而不是每一级临时生成一批题。现行机器合同 `ownward.kernel-iteration-blind-suite-contract/v2`（身份 `ccd1522f…34e`）只声明生成与质量准入职责，运行时从统一外部智能目录解析所选适配器、模型和档位；评价批次只接收显式 candidate 与 baseline 及其冻结收据，不含供应商、V2/V0 或 Stage 5 硬编码。既有 V2 套件继续由封存目录内的 v1 合同快照验证，切换外部智能不会重建或改写题目。其历史 Terra/medium 正负对照资格 plan/result 为 `bb275cdb…aa7e` / `45345923…3372`。V2 preparation plan `c875143c…53d7` 首轮生成 95 槽并接受 84 槽，第二轮只替换 11 个拒绝槽；最终 95/95 通过唯一答案、证据充分、无表面捷径、真实性、难度和评分区分力检查，封存套件 `346852e3…ed5` 与公开收据 `2200e3fa…288`。准备共 106 次生成、8 次准入调用，零重试、零限流、零中断；candidate/baseline 执行均为 0，正式 state SHA-256 前后保持 `b299ce4f…1c6f`。公开收据不含可逆题面、真值、答案、case ID 或 V0 角色；同身份恢复为零模型、零产品执行。旧逐关控制器历史不被改写，但已退出当前候选判定入口。

终态 read frontier 不在请求期重开或扫描权威正文，也不复制固定 passage：索引阶段只为每个来源维护有界 bottom-k sketch 与既有词法分数，融合边界以“前四 fused rank + 两个现有深来源 + 固定 sketch 多样性”确定 8 个读取名额并保持其余原序。候选 subject/receipt/binary 为 `99046f78…d18d` / `f39f1148…d92d6` / `0cacc994…ea1a`；38/46/54/62 资产、四 worker 的既有 36 次零模型请求 p95 为 `485.404 ms`，18/18 已知质量保护全部正确且同身份恢复零模型、零产品执行。显式 batch/freeze 为 `1ec95b63…d37a` / `176fef5c…1d84`。5 题候选 5/5、事实完整、p95 `531 ms`，同尺 V0 为 2/5 且 3 题证据未读；15 题候选仍为 15/15、事实完整，不可改写原结果保留 mean/p95 `616.733/1734 ms` 与 `retrieval_p95_confirmation_required`，V0 对照未消耗。

针对该单尾，在接触测量结果前固定同一候选、材料、prepared data、协议、传输、`24/8/24000` 预算和 `553 ms` 门；一次性独立非重合测量以 20 个单 worker 请求得到 mean/p95 `220.055/287.368 ms`，以 80 个四 worker 请求得到 `340.591/444.443 ms`，最大请求 `551.627 ms`；38/46/54/62 四组各 20 个样本的 p95 分别为 `390.610/541.257/364.717/411.667 ms`。决定性尾部同请求 Search/Evidence Search/Read/协议均值为 `338.815/63.042/97.176/0.558 ms`；四 worker GC 暂停总计 `29.67 ms`、scheduler runqueue 最大 1，冻结候选未暴露 mutex/block profile，故锁等待明确记为不可直接观测且没有用协议开销猜测替代。全部选择轨迹和目标交付稳定，最大读取 8、上下文 4090，零模型、零超时、零传输失败，prepared data 和正式 state 逐字不变。该充分样本与既有 36 请求结果共同证明 15 题单尾不是可重复的候选回归，按一次性有界确认保留当前候选且不重跑 5/15；场景专用控制器、Schema 和运行结果不进入长期依赖或恢复入口。机器终态只保留不可逆聚合、判定依据与通用结论。

通用版本套件执行器原先只能从 `passed=true` 的前级继续，不能消费不可改写原结果之外的充分样本裁决；当前入口现以跨版本通用的前级继续收据同时绑定 suite、evaluation batch、候选、原 plan/result 和目标级别，错绑、硬失败、质量证据不完整或正式 state 漂移均失败关闭。收据 `315fbc07…d83b` 与 15 题原结果共同进入 25 题 plan `91b8feeb…1dd2`，没有重写 5/15、重做资格或准备材料。25 题候选执行 75 次 Codex 调用，全部首次成功、零限流、零中断、零 worker 重启；20/25 回答正确，5 项首缺口均为目标证据已搜索返回但未读取，事实交付缺失 5，时间/冲突正确率 `0.75/1.0`，检索 mean/p95 `443.160/657.000 ms`。四项绝对门同时失败后在 `353.299 s` 停止，V0 和 50 题未运行；结果 `0fdacb67…1407`（SHA-256 `1030352a…6419`）不含可逆题面、答案或 case ID，scratch 已销毁，正式 state 仍为 `b299ce4f…1c6f`。该聚合只证明首个通用偏离边界是 `target_evidence_not_read`，具体机制仍需独立非重合材料复现，禁止直接把现象写成内核根因或原样重跑本分区。

当前 OpenCode Go / Qwen3.8 Flash evaluation batch `ec06bed0…70fe3` 在同一套件和冻结 subject 上形成 5 题 plan/result `2a1499d4…919f` / `ac0ab428…2f62`：候选与 V0 均为 5/5，事实交付完整，时间/冲突、相对门及候选检索 P95 `469<=553 ms` 全部通过；候选/V0 各 15 次模型调用，各一次有界重试和一次 worker 重启，无限流或中断。终态只因总墙钟 `546.098>406 s` 被标为评测进程失败，该旧上限来自 Codex/Luna 运行身份，不构成内核失败。进程预算 `a17e733e…6a440` 以该不可改写结果为当前外部智能的纯成本校准，保持质量合同、候选、基线与 `553 ms` 内核门不变，并继续受四级累计 `9000 s` 上限约束；它只授权复用 5 题完整证据进入 15 题，不允许把进程耗时改写为候选通过依据。

当前控制器补齐一次性 Reader 局部恢复：只有每个失效题都机械证明为“目标已在剩余读取/工具预算内返回但未读”或“同一索引可由直接问题探针取回、仅 Reader 搜索选择遗漏”时，才失效对应 Reader、Judge 与诊断派生结果；语义、索引、其他题和正式 state 保持不动，任何未归入两类的缺口失败关闭。混合归因与前级继续恢复的定向测试通过；独立 30 来源强干扰复现 plan/result `acc739de…dc69` / `3df9f1f9…a112` 为 1/1，未复现内核召回失败。

新 15 题 plan/result `fe31dbe8…ab18` / `07dbdbc4…7e65` 在一次 Reader 局部恢复后为 15/15，事实交付完整，时间与冲突正确率均为 1.0；唯一失败是 P95 `3047 ms` 的非候选尾值。既有同候选二进制的独立 20 个单 worker 加 80 个四 worker 请求为 P95 `444.443 ms`、最大 `551.627 ms`，继续收据 `d2991ca3…8e51` 因而只绑定当前结果授权进入 25 题，没有改写原结果或重测时延。

25 题 plan/result `555ccde3…a097` / `3d36efd6…0914` 在一次 Reader 局部恢复后事实交付 25/25、时间与冲突正确率 1.0，但首答 23/25，首个失败在候选上下文 3/3 稳定、完整 oracle 3/3 稳定正确且 Judge 对照通过，归因为 `candidate-context-failure`；检索 P95 `2641 ms` 也超过充分样本门。独立、非重合的最终回答充分性确认材料 plan/result `7196fb1f…6c5c` / `9805a277…3687` 质量仍为 2/2、事实交付完整，只因旧开发墙钟门未通过，未复现候选上下文缺陷；因此当前证据不授权修改 V2 或从 5 题重启，50 题未运行。正式 state 始终为 `b299ce4f…1c6f`。
...
5. **盲测**：每个内核大版本在接触候选评价前，由独立流程一次性生成、逐槽先验准入并私密封存完整 5/15/25/50 题版本套件。候选冻结后，以显式 evaluation batch 按分区顺序评价 candidate；candidate 达到绝对门槛后才评价同一分区的 baseline，判定不可逆时立即停止。候选、baseline、版本套件和评价控制器分别寻址，不得硬编码具体版本角色。
6. **失败反馈**：每次失败必须产出首个可泛化根因、负责方向、受影响证据和下一验证点；候选评价只公开不可逆聚合，私密版本套件和逐槽准入证书在大版本生命周期内保留且不可进入开发集或回归集。只有被先验拒绝或缺失的槽位可以局部替换，已接受且身份未变的槽位不得重复生成或复审。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | Stage 6 拒绝项的独立修复与 Stage 3/4 定向重闭合已完成；当前工作树候选尚未成为干净提交，因此 Stage 5 和该候选的 5/15/25/50 尚未启动。版本套题及其先验质量准入继续有效 |
...
| V2 候选 | 当前工作树候选 subject `460e3f82…b5fd`、内核世代 `44806167…fe1`、内核效果 `68552054…b86a`、组合 `ca4b5ce2…128a`、二进制 SHA-256 `dca56aa5…f0d3d`；它只存在于独立非正式制品，没有切换当前产品、V0/V1 或获得晋升资格 |
| 当前根因 | 已证首个偏离位于内核证据交付边界：短来源会丢失已经存在的查询相关度，预算内短权威正文又会被有损语义摘要替换。两者分别由固定读取前沿与公开摘要边界的独立反例锁定；修复后 3 个 diagnosis、2 个 confirmation、4 个 regression 均未再复现，旧 25 题原结果仍不可改写 |
| 方向状态 | Stage 3 交付根因已关闭；Stage 4 当前候选开发 4/4、固定回归 8/8，38—62 资产四 worker 的 80 次零模型复核 p95 `506.432 ms <= 553 ms`，选择与必要来源交付稳定。表示、语义和存储成本通过直接依赖迁移继续有效；Stage 5 等待干净候选重绑 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效；旧 batch 的 5/15/25 结果只绑定旧候选并保持不可改写，不能作为 subject `460e3f82…b5fd` 的关卡结果；当前候选尚未开始 5 题，50 题未运行 |
| 当前证据 | 修复终态 `.tmp/kernel-v2-major-iteration/stage6-context-delivery-repair-v2/answer-sufficiency/ae4d94fe…78bb/result.json` 身份 `bd08c8c5…e5a1`，3/3 diagnosis、2/2 confirmation、4/4 regression 全过，Reader/Judge 接受 30/30 且逐字重放成立；开发/回归结果身份 `b68d0359…0ed7` / `0463da09…c125`；当前候选充分样本时延结果身份 `d65938d6…c950`。同身份恢复均为零模型、零产品执行，正式 state 始终为 `b299ce4f…1c6f` |
| 下一验证点 | 协调者先独立复核并提交当前内聚工作树；随后只对新干净候选重建制品、重绑并重闭合 Stage 5 的真实失效证据，再创建该候选的 evaluation batch，按同一封存套题依次运行 5/15/25/50。提交前禁止以 modified 工作树冒充正式内部验收候选 |

恢复时只复用身份和直接依赖仍有效的结果。V2 代码或专属直接依赖变化只失效相应的 V2 证据；共享评测数据、工具、模型、提示词、评分器、配置或环境变化，只失效依赖该条件的 V2 比较证据。V0/V1 的制品、运行能力和各自在原合同下成立的既有证据不因 V2 变化而失效；在新数据或新评测合同下运行 V0 只是在该条件下建立对照，不是重验或改写 V0。候选冻结后，V2 或任一冻结直接依赖变化必须重开阶段 5，并失效依赖该候选的 evaluation batch/result；它不得使版本套件、逐槽准入证书或不依赖候选的准备证据失效。相同身份的中断可以从原子检查点继续；文档和无关项目变更不得迫使任何稳定内核重测。
...
- 同一版本套件的 5、15、25、50 题分区依次通过；95 个槽位先验质量合格、与已有数据不重合，私密套件在版本生命周期内保持封存且不泄露给开发或候选，累计证据证明未知数据泛化和 V0 能力保护。
...
5. 审查效果与抗拟合证据。开发集必须提供分钟级真实端到端反馈，固定回归必须按 V0 已正确能力的类型、复杂度和关键结构代表性覆盖，并保护经同尺重证且与失败机制可分离的 V1 收益，局部代理只能诊断；开发与回归材料必须独立构造，正式测试内容不得进入开发、回归或盲测生成。每个内核大版本在候选评价前必须由独立流程一次性生成、逐槽准入并私密封存完整 5、15、25、50 题版本套件；候选按同一套件逐级运行，裁决失败时只失效该候选评价、保留通用根因并退回完整迭代，已接受槽位和证书不重生不复审。评测流程失败只阻断下一关，既有候选通过事实保持不可改写，直至流程修复和独立非候选资格完成。
