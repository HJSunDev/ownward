...
状态：subject `a0a59771…117` 的 Stage 4 与干净 HEAD `a74a2e1…65e9` 封存的 Stage 5 制品继续有效；frontier、core、qualification、full 四层同身份恢复均为零执行复用。Stage 6 的终态 read-frontier 候选 `99046f78…d18d` 已完成 18/18 已知质量保护；5 题分区通过，15 题不可改写原结果保持 15/15 回答和事实交付正确，单次 p95 `1734 ms` 已由一次性独立充分样本确认不是可重复候选回归且没有重跑 5/15。同一冻结套件与 evaluation batch 的 25 题分区随后唯一执行一次：候选 20/25，首缺口为 5 项 `target_evidence_not_read`，时间正确率 `0.75`、冲突正确率 `1.0`、检索 p95 `657 ms`，同时未过质量、交付、时间和时延绝对门，故候选真实拒绝；V0、50 题均未运行。Stage 6 未完成，下一工作包必须从“目标证据已返回但未读取”的首个通用边界回到完整迭代闭环，正式 LongMemEval-S 继续禁止。
...
| 6. 一次性盲测 | 进行中（当前候选在 25 题被拒绝） | 不含原始题面的四级累计报告 | 通用版本套件合同 `33a11353…00e` 已一次性封存 V2 的 95 题与 95 份逐槽准入证书，分区为 5/15/25/50；套件身份 `346852e3…ed5`，公开收据 `2200e3fa…288`。evaluation batch `1ec95b63…d37a` 的 5 题通过；15 题不可改写原结果 `59aea598…0b5d` 保留 p95 单尾并由一次性独立充分样本裁决为继续同一候选，禁止重跑 5/15。通用前级继续收据 `315fbc07…d83b` 与原结果同时进入 25 题 plan `91b8feeb…1dd2` 的直接依赖；25 题结果 `0fdacb67…1407` 因 5 项事实未交付、首答与时间错误及充分分布 p95 `657 ms` 被拒绝。V0、50 均未运行；正式 state 未写，下一步回到完整迭代闭环，不得原样重跑该分区 |
...
Stage 6 的终态架构已改为版本级完整套件，而不是每一级临时生成一批题。机器合同 `ownward.kernel-iteration-blind-suite-contract/v1`（身份 `33a11353…00e`）把生成、质量准入、存储和评价控制器拆成独立身份；评价批次只接收显式 candidate 与 baseline 及其冻结收据，不含 V2/V0 或 Stage 5 的角色硬编码。Terra/medium 固定正负对照资格 plan/result 为 `bb275cdb…aa7e` / `45345923…3372`。V2 preparation plan `c875143c…53d7` 首轮生成 95 槽并接受 84 槽，第二轮只替换 11 个拒绝槽；最终 95/95 通过唯一答案、证据充分、无表面捷径、真实性、难度和评分区分力检查，封存套件 `346852e3…ed5` 与公开收据 `2200e3fa…288`。准备共 106 次生成、8 次准入调用，零重试、零限流、零中断；candidate/baseline 执行均为 0，正式 state SHA-256 前后保持 `b299ce4f…1c6f`。封存目录保存不可变合同快照；全新进程只凭 plan identity 终态复用为零模型、零产品执行，公开收据不含可逆题面、真值、答案、case ID 或 V0 角色。Acceptance Suite 354/354、LongMemEval-S 适配器 46/46、`check`、`self-check` 与差异检查均通过；任务相关后台进程为 0。旧逐关控制器历史不被改写，但已退出当前候选判定入口。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | Stage 4 与 Stage 5 继续有效，full 为 24/24。Stage 6 完整 95 题版本套件保持有效；当前 read-frontier 候选通过 5 题、15 题经有界确认保留，但在唯一一次 25 题分区被拒绝。V0 与 50 题未运行，正式 community 继续禁止 |
...
| 当前根因 | 25 题中 5 项的首个可证偏离边界为 `target_evidence_not_read`：目标证据已由搜索返回但未进入读取/回答链，同时出现事实交付、首答、时间和充分分布检索尾延迟失败。该现象负责方向为信息组织与交付，但具体排名、冗余、调度、深度或预算机制尚未由独立非重合材料区分，必须保持“待证明机制”，不得直接冒充根因 |
| 方向状态 | read-frontier 的已知质量链、38—62 资产时延及 15 题有界确认仍是有效历史证据；25 题未知材料证明当前候选没有泛化通过，Stage 6 停止。下一工作包回到完整迭代闭环，从独立端到端复现定位 5 项未读取的首个真实机制，不得原样重跑盲测内容 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 含唯一冻结的 5/15/25/50 分区，身份依次为 `0c31d4c0…836f` / `6beed6d0…7359` / `0a341a9e…3e59` / `4f17a743…76`；95 个槽位均有逐槽机械证明和语义准入证书。当前 batch `1ec95b63…d37a` 的 5 题通过，15 题原结果由独立有界确认保留同一候选，25 题候选被拒绝；V0 与 50 题未运行，旧逐关结果只作历史诊断 |
| 当前证据 | Stage 5 四层证据继续有效；read-frontier 真实规模证据 `306ada79…6946` p95 `485.404 ms`，已知质量保护 18/18。5 题 result `66671a12…5331` 通过，15 题原 result `59aea598…0b5d` 不改写；继续收据 `315fbc07…d83b` 只授权同候选进入 25 题。25 题 result `0fdacb67…1407` 不可逆记录 20/25、事实缺失 5、时间正确率 0.75、p95 657 ms，V0 未运行。机器终态 `feb4de00…3f25`，正式 state 始终为 `b299ce4f…1c6f` |
| 下一验证点 | 停止 Stage 6，不运行 50 或正式 LongMemEval-S，不重跑 5/15/25。下一工作包只接收 25 题不可逆聚合，在不读取或恢复盲测内容的独立材料上机械复现“证据已返回但未读取”，区分排名、冗余、调度、证据深度和预算适配并选择首个可证通用根因；随后回到完整 Stage 3—5 迭代，不得在盲测层直接修候选 |
