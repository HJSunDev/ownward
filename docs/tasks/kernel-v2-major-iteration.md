...
状态：评测纠偏和外部智能切换收尾已完成。V0/V1/V2 主动检索同尺非盲比较均为 `20/20`；终态 V2 候选在同质量下具有最低墙钟、检索均值、P95 与数据量，成为后续唯一候选基线。Stage 3—5 中不依赖旧观察器的证据继续有效；95 题版本套件及准入证书保持封存。候选与 V0 已通过同一 OpenCode Go / Qwen3.8 Flash 角色绑定和无模型预检；5 题质量、相对基线及内核性能门已通过，评测进程预算已按当前外部智能独立校准，下一动作是 15 题关。旧被动评测结果只保留审计意义。不运行正式 LongMemEval-S 或晋升。
...
| 6. 一次性盲测 | 5 题通过，等待 15 题 | 不含原始题面的四级累计报告 | 套件 `346852e3…ed5` 的 95 题及逐槽准入证书保持封存。当前 evaluation batch `ec06bed0…70fe3` 的 5 题候选与 V0 均为 5/5，事实交付、时间/冲突及相对门全部通过，候选检索 P95 `469<=553 ms`。原 `406 s` 墙钟来自旧外部智能，只造成评测进程失败；独立进程预算 `a17e733e…6a440` 不改质量、候选、基线或内核性能门，绑定本批次并保持四级累计 `9000 s` 设计上限，授权直接进入 15 题而不重跑 5 题 |
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 产品忠实评测纠偏完成；V2 被选为后续唯一候选基线。Stage 4/5 中不依赖旧观察器的证据继续有效；Stage 6 题目及先验质量准入有效，旧候选执行与裁决失效 |
...
| 当前根因 | 评测观察器越权已纠正；旧 `target_evidence_not_read` 属于评测流程现象，不能作为 V2 内核根因 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效；当前 batch `ec06bed0…70fe3` 的 5 题质量、相对基线和内核性能门均已通过，15/25/50 未运行 |
| 当前证据 | Stage 5 四层证据继续有效；read-frontier 真实规模证据 `306ada79…6946` P95 `485.404 ms`，已知质量保护 18/18。当前 5 题 result `ac0ab428…2f62` 中候选/V0 均 5/5，候选 P95 `469 ms`；进程预算 `a17e733e…6a440` 只校准外部智能墙钟并保持累计 `9000 s` 上限。旧 5/15/25 结果保持历史诊断，正式 state 始终为 `b299ce4f…1c6f` |
| 下一验证点 | 复用当前 5 题终态与进程预算直接执行 15 题；只有真实候选失败才返回内核优化，评测进程或外部智能异常不得误伤候选，正式 LongMemEval-S 继续禁止 |
