...
状态：阶段 4 进行中。V0 是唯一有效评测基线；当前产品仍运行迁移后保持 V1 行为的组合，V1 未晋升。首个独立 V2 候选已关闭“必要同源片段未完整交付”问题链，但阶段 4 尚有其他真实瓶颈待按冻结问题池继续处理，未进入整体验证或盲测。
...
| 4. V2 候选迭代 | 进行中 | 独立 V2 内核世代及净提升证据 | 首个候选已消除必要同源片段交付缺失并通过本问题链门槛；继续沿冻结问题池逐项处理剩余真实瓶颈，每次只运行受影响证据，无效路线及时淘汰，全部瓶颈关闭后才进入阶段 5 |
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 阶段 4 进行中；必要同源片段交付缺失问题链已关闭，阶段 5 未开始 |
...
| V2 候选 | subject `784fb1bf…be69`、内核世代 `7893f0b3…28c4`、内核效果 `557921aa…d5e1`；自包含组合与向量直接依赖可独立运行，未替换当前产品 |
| 当前根因 | 首根因已关闭：固定非重叠片段会拆开字段与值，且相邻高分片段会重复消耗有限证据名额；候选以有界双向连续窗口和仅对查询命中片段的最大边际来源覆盖消除，下一根因尚未按问题池选择 |
| 方向状态 | 信息表示与组织：当前问题链已关闭但方向仍在阶段 4；检索架构、数据结构与存储：优化；语义能力、执行架构与状态：保护。不得把首链收益冒充其他方向关闭 |
...
| 当前证据 | 阶段 2、3 证据继续成立；阶段 4 合同 `43625b14…0c90`、开发结果 `f6000fa7…558e`、回归结果 `299980ae…3e72` 与终态 `3412a348…4d1b` 共同绑定同一 V2 subject，正式 state 摘要仍为 `3c7826e0…fcdc` |
| 下一验证点 | 协调者先独立复核并提交本候选；随后仍在阶段 4，从冻结问题池选择覆盖价值最高且尚未关闭的一个可证瓶颈，只运行其受影响开发/回归证据。不得进入阶段 5、候选盲测或正式 LongMemEval-S |
