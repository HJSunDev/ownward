...
状态：V2 候选 `460e3f82…b5fd` 保持冻结，95 题版本套件及准入证书保持封存。最新 5 题以主动检索真实运行并 `5/5` 正确、事实交付完整、时间正确率 `1.0`；原结果 `b33ef6db…297b` 只因把单次内核调用口径的 `553 ms` 错用于单题多轮主动检索累计时延而拒绝。纠正收据 `e53b1cc3…107a` 已以零模型、零产品执行离线重判候选绝对质量通过；原始结果不改写，失败归属评测流程。下一步只复用已保留的候选执行并运行同题 V0，完成相对裁决；不得重跑候选。
...
以 V0 为唯一对照，完成一次真实泛化的内核大版本迭代：候选在信息组织质量、检索与最终回答质量、端到端效率上形成整体大幅跃升，V0 已有正确能力无实质退化，并保持或提高经同尺重证且与失败机制可分离的 V1 收益；通过固定回归和低成本内部保护后冻结候选，依次通过 5、15、25、50 题版本级盲测，再补齐一次最终内部验收，达到可运行正式 LongMemEval-S 的标准。
...
| 6. 一次性盲测 | 5 题候选质量通过，等待同题 V0 | 不含原始题面的四级累计报告 | 主动检索原始结果 `b33ef6db…297b` 为 5/5；`e53b1cc3…107a` 只纠正错误性能裁决并以零模型、零产品执行确认绝对质量通过。下一动作仅运行同题 V0 完成相对裁决；候选不得重跑。旧被动 15/25 题结果不具产品证明力 |
...
阶段按直接依赖推进和失效：V2 比较合同或共享评测条件变化，只重开阶段 1 的受影响部分及依赖它的 V2 比较证据，不使 V0/V1 内核或其既有证据失效；验证实现、数据质量准入或校准失败只重开阶段 2 的受影响部分，不计为候选失败；开发、回归或低成本内部保护失败回到阶段 3 至 5。盲测候选失败只失效该 evaluation batch 的候选结果并保留通用根因，版本套件、逐槽准入证书、共享条件未变的 V0 分区结果和其他不依赖候选的证据继续有效；修复后的新候选必须从同一套件的 5 题分区重新开始。最终内部验收失败只重开首个真实失效分段及其下游；任何失败都不得无条件重做仍有效的前置工作。
...
4. **盲测前冻结**：只有开发、完整回归、frontier 完整模式和低成本端到端保护已显示大幅提升且无实质退化的候选才冻结身份；不得提前运行资格或完整专项集。
5. **盲测**：每个内核大版本在接触候选评价前，由独立流程一次性生成、逐槽先验准入并私密封存完整 5/15/25/50 题版本套件。候选冻结后，以显式 evaluation batch 按分区顺序评价 candidate；candidate 达到绝对门槛后才评价同一分区的 baseline，判定不可逆时立即停止。候选、baseline、版本套件和评价控制器分别寻址，不得硬编码具体版本角色；共享条件未变的 baseline 分区结果跨候选复用。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | V2 候选保持冻结；最新主动检索 5 题候选执行已离线纠正为绝对质量通过，等待只运行同题 V0 完成相对裁决。资格集与 full 10/24 现场继续暂停 |
...
| 迭代合同 | `ownward.kernel-iteration-comparison/v5` 政策修订 `bb5fb3b4…bedb` 与 `ownward.kernel-iteration-baseline-facts/v1` 已冻结；错误的 `553 ms` 累计检索门、旧被动整题墙钟及旧被动质量门只保留历史诊断。V0、当前产品和 V2 subject 独立，Git 仅作审计来源 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效。当前 batch `d4bcfca1…8f87b` 的最新 5 题候选为 `5/5`；V0、15/25/50 尚未在纠正后完成 |
| 当前证据 | 最新 5 题 plan/result 为 `2a0f4f3a…0ee6` / `b33ef6db…297b`；候选报告 `9f4687e0…0366` 与检查点 `825f7f1e…cddb` 保留完整现场。纠正收据 `e53b1cc3…107a` 证明主动检索、`5/5` 质量通过、零模型与零产品离线重判；原始结果保持不可改写 |
| 下一验证点 | 复用已保留的 5 题候选执行，只运行同题 V0 并完成相对裁决；通过后按 15/25/50 推进。不得重跑 5 题候选或因历史 `553 ms` 门再次停下 |

恢复时只复用身份和直接依赖仍有效的结果。V2 代码或专属直接依赖变化只失效对应流水线分段及其下游；查询、读取或回答侧变化不得失效组织准备、语义组织和索引结果。共享评测数据、工具、模型、提示词、评分器、配置或环境变化，只失效依赖该条件的比较证据。V0/V1 的制品、运行能力和各自在原合同下成立的既有证据不因 V2 变化而失效；同一套件分区和共享条件下的 V0 对照跨候选复用。候选变化只失效其 evaluation batch/result，不得使版本套件、逐槽准入证书或候选无关证据失效。相同身份的中断从原子检查点继续；文档和无关项目变更不得迫使任何稳定内核重测。
...
8. 审查执行效率。依据已有机器实测和新增机制的保守成本模型核算正常路径与失败路径；日常归因、开发和受影响验证必须保持分钟级。证据必须按流水线分段失效，查询侧变化不得重算组织、语义或索引；同一套件与共享条件下的基线结果必须跨候选复用；高层控制器不得因具体修复增加专用角色；所有外部智能角色只能经统一端口装配。重复准备、重复验证、无效等待、串行交接、没有新证据的高成本运行和过晚淘汰都必须消除；四级盲测无法在既定质量下满足成本上限时，必须先修正执行路径，不能降低标准。
