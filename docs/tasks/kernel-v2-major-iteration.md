...
状态：评测纠偏和外部智能切换收尾已完成。V0/V1/V2 主动检索同尺非盲比较均为 `20/20`；终态 V2 候选在同质量下具有最低墙钟、检索均值、P95 与数据量，成为后续唯一候选基线。Stage 3—5 中不依赖旧观察器的证据继续有效；95 题版本套件及准入证书保持封存。候选与 V0 已通过同一 OpenCode Go / Qwen3.8 Flash 角色绑定和无模型预检，Stage 6 可从 5 题关重新执行；旧 5/15/25 题结果只保留被动评测审计意义。不运行正式 LongMemEval-S 或晋升。
...
| 6. 一次性盲测 | 已具备重新执行条件 | 不含原始题面的四级累计报告 | 现行准备合同 v2 `ccd1522f…34e` 只冻结 Generator 与质量准入职责，模型和档位由所选外部智能角色配置提供；既有套件保留生成时的 v1 合同快照，无须重建。95 题及逐槽准入证书已封存为 5/15/25/50 四个分区，套件身份 `346852e3…ed5`，公开收据 `2200e3fa…288`。旧批次及结果只保留被动评测审计意义。新 evaluation batch `fc6e8ab4…cb00` 复用同一冻结 V2、V0 与版本套件，候选和基线均绑定默认 OpenCode Go / Qwen3.8 Flash 角色；批次、制品、环境、协议、角色、所选适配器及全部封存分区已完成无模型校验，尚未运行任何新关卡。下一动作是从 5 题关开始不可逆评价 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效；5/15/25 的旧候选结果只作被动评测历史诊断，未证明通过或拒绝 |
...
| 下一验证点 | 以 `external-agent-progressive/v1` 和同一冻结 V2 重新执行 Stage 6 盲测关卡；旧候选结果不得复用，正式 LongMemEval-S 继续禁止 |
