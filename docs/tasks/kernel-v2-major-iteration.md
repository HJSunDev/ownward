...
状态：阶段 3 已完成，阶段 4 待开始。V0 是唯一有效评测基线；当前产品运行的是迁移后保持 V1 行为的组合，V1 未晋升；V2 候选尚未建立。比较合同、通用非正式执行闭环、一次性盲测机制、聚合问题池、独立开发/回归材料和五方向决策均已在接触任何 V2 候选结果前冻结。
...
| 迭代能力 | `benchmarks/acceptance/suite/kernel_iteration_run.py` 是唯一通用非正式入口；`iteration/v2/comparison-contract.json` 冻结比较政策，`iteration/v2/validation-contract.json` 冻结执行与盲测职责，`iteration/v2/stage3-contract.json` 冻结聚合问题池、独立材料、门槛与收益裁决规则。原子计划/检查点可恢复且不能写正式 state；V1 专用入口已退出生产路径 | 阶段 4 直接复用阶段 3 已封存的开发、固定回归和受限非候选诊断证据；尚未建立 V2 候选或候选结论 |
...
| 4. V2 候选迭代 | 待开始 | 独立 V2 内核世代及净提升证据 | 候选沿最高价值问题链持续取得端到端净提升；每次修改只运行受影响证据，无效路线及时淘汰，全部真实瓶颈达到预先门槛 |
...
| 方向               | V1 已知观察                                                                  | 初始状态 |
| ------------------ | ---------------------------------------------------------------------------- | -------- |
| 信息表示与组织结构 | 独立案例在目标来源已返回并读取后只交付 5 项必要事实中的 2 项，直接导致回答错误 | 决策：优化；阶段 4 首方向 |
| 检索架构与算法     | 当前产品开发/回归检索均值分别为 `242.25/247.875 ms`，仍须受 V0 同尺边界约束并保证证据交付 | 决策：优化；与首方向联动保护 |
...
| V2 候选 | 未建立 |
| 当前根因 | 已证首根因：目标来源已搜索返回且已读取，但按需片段没有交付完成回答所需的全部同源事实；当前独立开发案例交付 2/5 项必要事实。负责方向为信息表示与组织结构 |
| 方向状态 | 信息表示与组织、检索架构、数据结构与存储：优化；语义能力、执行架构与状态：保护。保护项仍须在后续候选上通过，不等于方向提前关闭 |
| 保护边界 | V0 固定回归 8/8 且事实交付完整；V1 长资产语义召回与批量耐久已同尺重证并可分离，细粒度证据和存储收敛未达到保护条件 |
| 冻结门槛 | 首方向必须消除独立开发集的片段缺失并至少将该错误减半（当前离散基线为 1，候选须为 0）；V0 固定回归保持 8/8，检索与资源不得越过比较合同边界，日常受影响反馈不超过 600 秒 |
| 盲测级别 | 候选盲测未开始；独立生成、准入、执行、评价、销毁、恢复机制及非候选五题校准已成立，校准数据不得用于后续候选判断 |
| 当前证据 | 阶段 2 预算冻结身份 `90d2b744…a224` 及其当前依赖验证仍成立。阶段 3 合同 `049c3e4a…e84e`、计划 `869633da…478d`、终态证据 `79051919…3543` 绑定问题池、4 个开发案例、8 个固定回归案例、两类 subject、共享条件和四份逐字恢复结果；正式 state 摘要保持 `3c7826e0…fcdc`，没有 V2 候选结果 |
| 下一验证点 | 阶段 4 只建立首个独立 V2 候选，优先消除“来源已读但必要同源片段未完整交付”根因；先达到开发集零片段缺失、V0 固定回归 8/8、长资产召回与批量耐久保护，再评估检索时延和存储门槛。不得同时启动第二方向或候选盲测 |
