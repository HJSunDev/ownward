...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 产品忠实评测纠偏完成；V2 被选为后续唯一候选基线。Stage 4/5 中不依赖旧观察器的证据继续有效；Stage 6 题目及先验质量准入有效，当前 5 题通过、15 题质量通过并有时延继续收据、25 题拒绝，50 题未运行 |
...
| V2 候选 | Stage 5 冻结候选仍为 subject `a0a59771…117`；其上形成的终态 read-frontier 候选 subject `99046f78…d18d`、内核世代 `6a729391…872`、内核效果 `71e80326…90f`、组合 `e6caa1ed…3de`、二进制 SHA-256 `0cacc994…ea1a` 经一次性有界确认继续作为 Stage 6 当前候选，但没有切换当前产品、V0/V1 或获得晋升资格 |
| 当前根因 | Reader 的预算内未读与直接问题可召回但搜索选择遗漏已归入一次性局部评测恢复；25 题剩余 `candidate-context-failure` 与充分样本检索尾值尚未在独立非重合材料上复现，不能据此修改 V2 |
| 方向状态 | V2 同尺非盲比较 `20/20`，累计墙钟 `261.608 s`、检索均值 `90.55 ms`、P95 `126 ms`、数据量 `730,994 B`，均优于同质量 V0/V1；候选保持冻结 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入继续有效；当前 batch `ec06bed0…70fe3` 的 5 题通过；15 题在局部恢复后质量 15/15，并由独立充分样本确认单尾；25 题首答 23/25 且 P95 失败，50 题未运行 |
| 当前证据 | Stage 5 四层证据继续有效；read-frontier 真实规模证据 `306ada79…6946` P95 `485.404 ms`，已知质量保护 18/18。当前 5 题 result `ac0ab428…2f62`、15 题 result `07dbdbc4…7e65`、继续收据 `d2991ca3…8e51` 与 25 题 result `3d36efd6…0914` 均保持不可改写；独立召回复现 1/1、最终回答充分性确认 2/2，均未复现盲测候选缺陷。正式 state 始终为 `b299ce4f…1c6f` |
| 下一验证点 | 保留 25 题拒绝终态且不原样重跑；下一工作包只能先冻结新的独立非重合复现合同，分别验证候选上下文充分性与完整消费者尾值。只有通用缺陷稳定复现才可修改 V2 并从同一套件 5 题重启；否则不得进入 50 题或正式 LongMemEval-S |
