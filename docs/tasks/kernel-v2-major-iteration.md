...
状态：阶段 4 进行中。V0 是唯一有效评测基线；当前产品仍运行迁移后保持 V1 行为的组合，V1 未晋升。独立 V2 候选链已关闭必要同源片段、多来源证据加载和检索时延：四个独立 worker 的系统级线程预算由错误的总计 `24` 个推理线程、`8` 个槽位收敛为产品原生 2/1 的 `8` 个推理线程、`4` 个槽位，完整消费者 p95 为 `356.673 ms`，在既有 `600 ms` 权威门和 `47 ms` 冻结误差外仍保留充分余量；全部质量、资源和恢复保护通过。尚未进入端到端资源成本、整体验证或盲测。
...
| 4. V2 候选迭代 | 进行中 | 独立 V2 内核世代及净提升证据 | 当前候选链已消除必要同源片段交付缺失、多来源证据加载缺失和检索时延；检索时延按真实 38—62 资产、四 worker 的完整消费者职责由 `596.211 ms` p95 降至 `356.673 ms`，通过 `p95 + 47 ms <= 600 ms` 并满足额外 20 ms 工程余量。端到端资源成本等冻结问题池剩余瓶颈关闭后才进入阶段 5 |
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | 阶段 4 进行中；必要同源片段、多来源证据加载与检索时延均已关闭，端到端资源成本尚未开始，阶段 5 未开始 |
...
| V2 候选 | 当前候选 subject `208982be…517f`、内核世代 `c84215c3…a664`、内核效果 `f20de76f…32d6`；它保留同源片段、多来源、惰性证据与连接复用，删除错误 6/2 overlay 并绑定产品原生 2/1。前序质量 subject `7b78a0ef…4fbd` 与失败诊断制品继续保留，不作为当前终态 |
| 当前根因 | 已关闭的时延根因是单进程 6/2 吞吐配置在四个独立 worker 下形成 24 个推理线程和 8 个槽位，超过本机 12 逻辑核并放大 Search 尾部；产品原生 2/1 把系统预算收敛为 8 个推理线程和 4 个槽位，完整 p95 降至 `356.673 ms` 且质量逐字稳定 |
| 方向状态 | 信息表示与组织当前链已关闭；检索架构的多来源与整体时延链已关闭；数据结构与存储的端到端资源成本仍待处理；语义能力、执行架构与状态继续保护 |
| 保护边界 | V0 固定回归 8/8 且事实交付完整；V1 长资产语义召回与批量耐久已同尺重证并可分离，细粒度证据和存储收敛未达到保护条件 |
| 冻结门槛 | 同源片段与多来源材料的必要事实缺失均须归零；V0 固定回归保持 8/8，长资产语义召回保持 1.0，读取/上下文/持久状态不增长；完整消费者检索 `p95 + 47 ms <= 600 ms`，即观测 p95 不高于 `553 ms`，纯内核 frontier p95 和正式四 worker 总墙钟仍按各自原口径独立判定，日常受影响反馈不超过 600 秒 |
| 盲测级别 | 候选盲测未开始；独立生成、准入、执行、评价、销毁、恢复机制及非候选五题校准已成立，校准数据不得用于后续候选判断 |
| 当前证据 | 阶段 2、3、同源片段、多来源、质量与恢复证据继续成立；同尺审计 `f2ef1f64…c8b3` 与迁移收据 `55131b81…0cec` 只纠正时延维度。系统预算合同/性能/终态为 `0555bdfe…6564` / `d4f859b8…0436` / `10b84de7…4caa`；旧真实规模 `72b6988e…33f4` 与尾部拒绝证据继续保留根因诊断，正式 state 仍为 `3c7826e0…fcdc` |
| 下一验证点 | `retrieval-latency` 已关闭；阶段 4 的唯一下一问题链为 `end-to-end-resource-cost`，必须另行冻结范围与门槛，本工作包未执行 |
