...
最新状态（2026-09-05 21:04）：用户已将轻量客户端 `opencode-go-api/v1` 确认为第三个正式外部智能并设为默认。关卡与最终测试统一采用 `.tmp/final-test-ready/execution.json`；默认 Qwen3.8 Flash 的语义/Judge 为 medium、Reader 为 xhigh，生成/准入沿用 xhigh。另两个实现及其角色与显式选择能力保留，不改写既有冻结证据。预检已分别核验进程池或进程内请求的并发与隔离。147 项相关检查、500 题官方环境与协议检查通过；此前轻量版 4/4 正确的原身份结果已精确恢复，报告、检查点清单、提交包逐字不变且零模型调用。正式启动入口为 `.tmp/final-test-ready/run-final.py`，计划为 `final-plan.json`，新结果目录为 `E:/Ownward/acceptance/longmemeval-s/runs/15eaa12c101c2eb307d20c4172c4582bacafef093442b26d2468940bc98205ff/formal-go-api`。准备已完成，按用户要求停止，未启动 500 题；原 `formal` 中 14 道结果与全部失败/中止检查点保留。未来测试使用当前配置，旧配置仅供历史恢复或显式选择旧实现。

状态（2026-09-05 20:51）：第三个外部智能实现已完成独立性复核：仅加载所选 driver，轻量客户端通过公共端口复用结构化校验，不依赖 OpenCode 实现；133 项接口、运行器与关卡检查通过。相同 4 道官方题、同一冻结 V2、同一 Qwen3.8 Flash 角色档位、4 题并发/8 路外部调用完整对照，两组均 4/4 正确。OpenCode 与轻量版解题阶段分别为 390.875 秒、297.375 秒（减少 23.92%），含启动收尾分别为 455.329 秒、305.750 秒（减少 32.85%）；客户端与 Python 宿主峰值内存分别为 4653.32MB、155.06MB，累计 CPU 分别为 1468.234、18.641 CPU 秒。证据：`.tmp/final-test-ready/go-api/comparison-20260905-203759/result.json`；两组逐题轨迹、评分与报告位于 `E:/Ownward/acceptance/longmemeval-s/runs/client-comparison-20260905-203759/`。这是单轮小样本提速测量，不能保证 500 题同比例缩短。正式测试保持暂停，原已完成结果与检查点保留。以下为此前阶段记录。

状态（2026-09-05 20:31）：用户要求暂停正式测试并改用轻量客户端。调度者已停净本轮进程，保留 14 道正确结果、1 道上游 HTTP 500 执行失败及 4 道中止题的检查点。`opencode-go-api/v1` 已完成：沿用同一 Go 凭证、Qwen3.8 Flash 与角色档位，一个宿主承载 8 路并发，无 OpenCode 子进程或新增依赖；只新增外部智能适配实现与装配登记，内核和业务运行流程不变。真实 8 路工具调用、原 20 条语义批次及检查点复用、原题 Reader/Judge 均通过，145 项新旧接口/运行器/关卡检查通过。配置与证据在 `.tmp/final-test-ready/go-api/execution.json`、`readiness.json` 和 `validation-20260905-202729/result.json`。验证进程实测约 41MB 内存，整轮提速尚未测定；正式测试仍暂停，未混用新旧身份或恢复运行。以下为此前阶段记录。

状态（2026-09-05）：新 V2 `5a1de835…1cce` 已通过 5/15/25/50 题。50 题中 c051、c064 的题目缺陷已修正并经真实入口验证正确；c047 的内核检索、目标证据返回与读取均正确，错误发生于外部 Reader 的答案组织，不属于内核关卡失败。旧来源故障题 c055、c066、c076 全部正确。V2 保持冻结，下一项工作只补齐最终内部验收与正式测试启动预检，不重跑关卡、不修改内核、不启动正式测试。

启动预检补充（2026-09-05）：运行接口、失败留痕和断点恢复已打通；0a995998 完整运行 274.671 秒、零重试，报告与恢复验证通过。总耗时现采用 `full_wall_policy=report-only`：预估不挡启动、整轮超过预期不截断，单题及单次调用保护保持，旧协议默认仍为 enforce。当前制品为 `.tmp/final-test-ready/state-rules/ownward.exe`，执行配置已更新，仅产品协作规则实现变化，内核实现未变。规则已去除事项状态的具体补丁，收敛为保留信息原意和确定程度、区分来源事实与新增推断。Reader 单独复验记录于 `reader-rules-pending-state-20260905-194656`：复用旧记忆与检索实现，从新产品取得规则，Reader 69.25 秒完成，仍回答 2、官方标准为 3，原判保留；不能宣称此题已被新规则修复，也不为该题添加提示或改判。运行就绪与逐题质量分别记录于 `.tmp/final-test-ready/readiness.json`。社区/进程/预检 21 项、LongMemEval 55 项、关卡与迭代 53 项检查及 MCP 规则合同测试通过。500 题尚未启动。

> 后续历史阶段表与执行记录不代表新候选的当前资格；与上述最新状态冲突时，以最新状态和对应机器结果为准。旧被动观察器结果仍不具产品证明力。

> 本文后续出现的 `553 ms` 均为历史诊断记录，不再是活动候选门槛。当前性能只按单次内核调用、单题主动检索累计和整题墙钟分别报告与裁决。
...
以 V0 为唯一对照，完成一次真实泛化的内核大版本迭代：候选在信息组织、检索与最终回答质量上形成真实跃升，V0 已有正确能力无实质退化，并保持或提高经同尺重证且与失败机制可分离的 V1 收益；效率守住真实场景的可用底线，在有证据的方向持续优化。通过固定回归和低成本内部保护后冻结候选，依次通过 5、15、25、50 题版本级盲测，再补齐一次最终内部验收，达到可运行正式 LongMemEval-S 的标准。
...
- 信息组织质量、检索与最终回答质量、端到端效率分别报告。质量优先，错误内容不能用速度抵消；效率超过真实场景可用底线同样不可接受，底线内不得因有限的耗时增加否决已证明的质量收益。按[迭代方法的评价优先级](../engineering/kernel-evolution-system.md#目标)裁决，不强制每次质量修复同时提速，不杜撰性能门槛。局部指标、资产命中或固定开发集满分不得关闭大版本。
- 已足够优秀或不限制整体结果的方向只建立回归保护，不强行修改；任何优化都必须反绑真实瓶颈。
- 日常反馈保持分钟级，只重跑真实失效部分；正式 500 题、重复环境准备和无新证据的高成本验证禁止进入本任务。
- 证据必须按组织准备、语义组织、索引、查询检索、证据读取、回答与裁判分段失效；查询侧候选变化不得重做仍有效的组织、语义或索引证据。高层验收只消费通用候选清单和分段汇总，不得因新增根因修改控制器或增加问题专属角色。
- 同一套件分区的 V0 结果按共享评测条件冻结并跨 V2 候选复用；只有 V0、套件、模型、提示词、评分器、配置或环境发生真实变化才重跑。所有评测角色只经 `ownward.external-intelligence/v1` 装配，供应商细节不得进入业务控制器。
...
| 3. 端到端归因与材料冻结 | 已完成 | 问题池、开发/回归清单、五方向决策 | 既有交付根因与定向证据已关闭；后续主动检索复核没有证明新的内核缺陷，不授权猜测式修改 |
| 4. V2 候选迭代 | 已完成并冻结 | 独立 V2 内核世代及净提升证据 | 当前候选既有质量、性能、资源和恢复证据保持有效；本轮评测纠偏不修改候选 |
| 5. 候选冻结与盲测准入 | 已完成（当前候选） | 干净候选身份、完整回归与低成本内部保护 | 提交 `67fdc76` 的独立 V2 制品由通用 Stage 4 闭合汇总 `d07f12ac…0c09f` 与候选清单 `fd5f2127…1ddaf` 证明质量、性能、资源和恢复门槛；不要求先完成 qualification/full 的外部智能准备或执行 |
| 6. 一次性盲测 | 5 题通过，等待 15 题 | 不含原始题面的四级累计报告 | 主动检索原始结果 `b33ef6db…297b` 为 5/5；现行关卡按原始聚合零模型、零产品离线重判通过。下一动作直接运行 15 题；不得重跑候选或运行同题 V0。旧被动 15/25 题结果不具产品证明力 |
| 7. 最终内部验收与测试交接 | 等待四级盲测 | 最终内部报告与最终测试就绪报告 | 四级全部通过且候选身份未变后，复用已通过的资格结果和完整专项集 10/24 检查点，只补齐真实失效或未完成证据；随后完成不运行正式题目的交接预检。正式 LongMemEval-S 与晋升仍不在本任务范围 |

阶段按直接依赖推进和失效：V2 比较合同或共享评测条件变化，只重开阶段 1 的受影响部分及依赖它的 V2 比较证据，不使 V0/V1 内核或其既有证据失效；验证实现、数据质量准入或校准失败只重开阶段 2 的受影响部分，不计为候选失败；开发、回归或低成本内部保护失败回到阶段 3 至 5。盲测候选失败只失效该 evaluation batch 的候选结果并保留通用根因，版本套件、逐槽准入证书和其他不依赖候选的证据继续有效；修复后的新候选必须从同一套件的 5 题分区重新开始。最终内部验收失败只重开首个真实失效分段及其下游；任何失败都不得无条件重做仍有效的前置工作。
...
当前 OpenCode Go / Qwen3.8 Flash evaluation batch `ec06bed0…70fe3` 在同一套件和冻结 subject 上形成 5 题 plan/result `2a1499d4…919f` / `ac0ab428…2f62`：候选与 V0 均为 5/5，事实交付完整，时间与冲突正确；候选/V0 各 15 次模型调用，各一次有界重试和一次 worker 重启，无限流或中断。原结果使用的 `553 ms` 累计检索门和 `406 s` 关卡墙钟门测量对象错误，只保留历史诊断；活动关卡不再加载进程预算或允许墙钟裁决候选与关卡推进。
...
当前 15 题聚合缺口的独立归因终态为 `.tmp/kernel-v2-major-iteration/stage6-unread-independent-attribution/attribution-result.json`（SHA-256 `1e366964…a9f2`）。不重合四题矩阵覆盖目标来源后排、多来源竞争、证据深度和时间冲突，目标排名最高到 7，全部目标均进入读取并 4/4 正确；强制长来源反例中，候选在第 2 名返回并读取目标、`evidence_search` 暴露三段必要证据，但 Reader 用尽 12 次工具调用后只读一段，首缺口为 `evidence_read_answer_incorrect`，不是盲测聚合的 `target_evidence_not_read`。同一材料的当前产品非候选控制以 8 次调用读完三段并正确回答。因此搜索返回或证据可用性不是该反例的根因；候选上下文下的 Reader 分配失败虽被观测到，却未在非候选控制中复现，尚不足以判定稳定外部评测缺陷。按失败开放规则，本轮不改候选、不改评测器、不运行新关卡，15 题拒绝保持；三次执行墙钟分别为 `407.047/217.390/89.250 s`，同身份恢复均逐字复用且零模型、零产品执行，正式 state 始终为 `99a416dc…c40d`。
...
4. **盲测前冻结**：开发、完整回归、frontier 完整模式和低成本端到端保护证明质量收益、既有能力保护与效率可用性后冻结候选；不强制每次修复同时提速，不得提前运行资格或完整专项集。
5. **盲测**：每个内核大版本在接触候选评价前，由独立流程一次性生成、逐槽先验准入并私密封存完整 5/15/25/50 题版本套件。候选冻结后，以显式 evaluation batch 按分区顺序只评价 candidate；任一级不满足绝对门槛即停止并返回优化。版本套件、候选和评价控制器分别寻址，关卡不绑定正式 Acceptance state，也不重复运行 V0。
6. **最终内部验收**：四级全部通过后保持候选不变，复用分段证据，只补齐固定内核、资格、完整 Ownward 专项及交接预检中尚未成立的部分。
7. **失败反馈**：每次失败必须产出首个可泛化根因、负责方向、受影响证据和下一验证点；候选评价只公开不可逆聚合，私密版本套件和逐槽准入证书在大版本生命周期内保留且不可进入开发集或回归集。只有被先验拒绝或缺失的槽位可以局部替换，已接受且身份未变的槽位不得重复生成或复审。

盲测正式启用前的历史非候选校准保留 5/15/25/50 题墙钟 `406/751/1097/1961` 秒及总计 `4215` 秒作为执行成本诊断；它们不参与候选判断、关卡推进或前级复用。当前 validation `059ea0ab…ccb6` 的两批 15 题非候选资格仍证明当时的准入质量与调度事实，但旧模型、墙钟和拒绝批次只作历史诊断，不约束当前统一外部智能执行。
...
| 字段 | 当前值 |
| --- | --- |
| 当前阶段 | V2 候选保持冻结；5/15/25/50 四级内核关卡全部通过。当前只补齐最终内部验收与正式测试启动预检 |
...
| 正式状态 | 提前执行后唯一 state SHA-256 为 `99a416dc…c40d`，保留 frontier/core/qualification 检查点和三条 V0 基线历史；隔离 full 目录保留 10 个 sealed result，但没有 full 顶层报告。上述现场不构成顺序合法的最终内部验收，本轮不改写、不删除；Stage 6 非正式盲测已运行 5/15，正式 community 未执行 |
| 迭代合同 | `ownward.kernel-iteration-comparison/v5` 政策修订 `bf3a3a2f…2229` 与 `ownward.kernel-iteration-baseline-facts/v1` 已冻结；错误的 `553 ms` 累计检索门、旧被动整题墙钟及旧被动质量门只保留历史诊断。V0、当前产品和 V2 subject 独立，Git 仅作审计来源 |
| V2 候选 | source subject `460e3f82…b5fd`、内核世代 `44806167…fe1`、内核效果 `68552054…b86a`、组合 `ca4b5ce2…128a`；干净重建包装 subject `2dc2af41…6c35`、二进制 SHA-256 `1b792d12…dbf`。它只存在于独立候选制品，没有切换当前产品、V0/V1 或获得晋升资格 |
| 当前根因 | 最新 5 题没有内核质量缺口；唯一拒绝来自评测流程把单次内核调用口径错用于多轮主动检索累计时间。V2 未修改 |
| 方向状态 | 既有 Stage 3/4 质量、资源和恢复证据保持有效；历史累计检索 `553 ms` 判定降为诊断，不再证明或否定候选性能。后续按单次内核调用、主动检索组合和整题体验分别取证 |
...
| 盲测级别 | V2 版本套件 `346852e3…ed5` 的 95 个槽位、机械证明和先验准入有效；当前候选已依次通过 5/15/25/50 题。关卡只裁决内核：题目缺陷与已证外部 Reader 错误不得处罚内核 |
| 当前证据 | 5/15/25 题 plan 分别为 `1b7ee5bf…0136`、`aa50af1b…b77f`、`e2ada29e…a2c5`；50 题证明 c055/c066/c076 已修复，修正后的 c051/c064 正确，c047 内核交付正确 |
| 下一验证点 | 复用现有检查点补齐最终内部验收与正式测试启动预检；不得重跑关卡、修改内核或提前运行正式 LongMemEval-S |

恢复时只复用身份和直接依赖仍有效的结果。V2 代码或专属直接依赖变化只失效对应流水线分段及其下游；查询、读取或回答侧变化不得失效组织准备、语义组织和索引结果。共享评测数据、工具、模型、提示词、评分器、配置或环境变化，只失效依赖该条件的比较证据。V0/V1 的制品、运行能力和各自在原合同下成立的既有证据不因 V2 变化而失效。候选变化只失效其 evaluation batch/result，不得使版本套件、逐槽准入证书或候选无关证据失效。相同身份的中断从原子检查点继续；文档和无关项目变更不得迫使任何稳定内核重测。
...
- 唯一 V2 迭代合同已在任何候选结果产生前冻结整体跃升门槛、不可退化能力、证据职责、失效图和成本边界；V0 历史结果未被改写，V2 只有在四级盲测通过后才补齐 Acceptance 内部检查点。
...
- 候选达到预先明确的信息组织、检索与最终回答质量目标，已有正确能力得到保护；效率守住有依据的可用底线并如实报告代价。大幅目标作用于具有提升空间的方向，不得以速度抵消质量缺陷，也不得将每次质量修复都必须提速设为结束条件。
- 固定回归、完整内部验收、恢复与失败开放均通过；局部代理或单一数据集未被用作完成证明。
- 同一版本套件的 5、15、25、50 题分区依次通过；95 个槽位先验质量合格、与已有数据不重合，私密套件在版本生命周期内保持封存且不泄露给开发或候选，累计证据在最终测试前未发现未知数据上的阻断性缺陷；V0 能力保护由盲测前完整回归证明。
...
1. 从预期结果反推任务完整性。执行本文全部阶段后，必须得到相对 V0 具有真实质量跃升、V0 已有正确能力无实质退化、效率满足真实场景可用底线并通过完整回归与四级盲测的同一冻结 V2 候选。遵循质量优先，不强制每次质量修复同时提速，不允许速度抵消质量缺陷；不能以流程完成、局部满分或小幅净收益冒充大版本目标成立。
...
3. 审查阶段与依赖是否完整。比较合同、验证能力、端到端归因、开发与回归材料冻结、五方向审视、候选优化、完整回归、低成本内部保护、候选冻结、5/15/25/50 题一次性盲测、最终内部验收、失败回退和最终测试交接都必须拥有清楚的输入、输出、状态、证据、关闭条件和唯一下一动作；高成本资格与完整专项只能在四级通过后补齐，验证机制不得要求在根因确认前猜测具体开发材料，不得出现无法派发、无法复核、无法恢复或提前结束的断点。
...
8. 审查执行效率。依据已有机器实测和新增机制的保守成本模型核算正常路径与失败路径；日常归因、开发和受影响验证必须保持分钟级。证据必须按流水线分段失效，查询侧变化不得重算组织、语义或索引；盲测只运行候选且不得绑定正式 state 或重复基线比较；高层控制器不得因具体修复增加专用角色；所有外部智能角色只能经统一端口装配。重复准备、重复验证、无效等待、串行交接、没有新证据的高成本运行和过晚淘汰都必须消除；四级盲测无法在既定质量下满足成本上限时，必须先修正执行路径，不能降低标准。
...
- **效果**：全文执行完成是否要求真实质量跃升，保护 V0 已有正确能力，并保持或提高经同尺重证且与失败机制可分离的 V1 收益；效率是否守住真实场景可用底线，既不以速度抵消质量缺陷，也不强制每次质量修复同时提速，无法以局部满分、小幅净收益或流程完成假结束。
