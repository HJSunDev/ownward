...
- 并发 8 的唯一一次最终四类代表预检共 187 个会话、12 个自然语义工作批、20 次 Codex 调用；20/20 首次完成，0 重试、0 限流、0 中断、0 transport 超时、0 worker 重启，12 批原序提交，20 个独立只读新 thread，报告与检查点恢复后逐字节不变，墙钟 132.359 秒。事实与载荷等价证据完整，路径为 `E:\Ownward\acceptance\longmemeval-s\runs\preflight\77600820-89e959e3ae7c92f1-production-profile`。
- 修正后的全量原始投影为 12,775.987 秒；加入 20% 正常波动、10% 有界重试与 3,600 秒恢复余量后为 20,053.902 秒，低于 20,400 秒硬上限。因此冻结并发 8，不测试 12，正式 500 题已解锁但本任务未启动。
