# 第一版验收基线依据

第一版采用 [Ownward Acceptance Suite v1](../../benchmarks/acceptance/suite/README.md)：固定内核基线证明资产保存、更新、备份恢复、派生重建、身份稳定、派生状态可销毁和模型可替换；固定 Ownward 专项数据集验证跨时间、多步关联、场景适用、信息更新及关系组织增益；固定版本 LongMemEval‑V2 Small 提供社区可比较的长期记忆质量—时效成绩。三层证据绑定同一候选且分别通过，不能互相替代。

内核前沿优化环使用公开、版本化的固定材料与冻结向量、语义输入，提供不超过三分钟的定向反馈和不超过十分钟的完整反馈。质量、时延和资源独立设防；候选只有在无受保护能力退化且至少一项预先定义的能力实质改善后才能进入固定资格集，资格验证通过后才成为有效内核基线。当前没有能够以同一输入、协议和环境复现的外部同类内核对照，因此内部结果不能被包装成外部前沿。

公开质量与时效只使用 LongMemEval‑V2 官方提交 `2cc8c540bdb87fe6761629b585e727e1c4704520`、Small 数据、Web/企业两个领域及官方评测和排行榜格式。Ownward 不接触答案或评测函数；公开基准暴露的问题只能抽象为通用能力并进入新版固定内核材料，不得针对官方题目增加规则。

资源前沿中，信息内核与完整交付仍分别观察，模型成本不能稀释内核判断，内核成本也不能隐藏用户实际承担的完整交付成本。`total-agent-memory v12.4.0` 的既有同机数据只保留为资源研究证据：由于能力合同、输入和执行协议不完全等价，不能换算成 Ownward 内核前沿或正式完成结论。

依据：

- [Ownward Acceptance Suite v1](../../benchmarks/acceptance/suite/README.md)
- [LongMemEval-V2 官方仓库](https://github.com/xiaowu0162/LongMemEval-V2)
- [total-agent-memory](https://github.com/vbcherepanov/total-agent-memory)
