...
正式基准固定官方仓库提交 `9e0b455f4ef0e2ab8f2e582289761153549043fc`、官方清洗数据集提交 `98d7416c24c778c2fee6e6f3006e7a073259d48f` 及其中的 `longmemeval_s_cleaned.json`。使用官方完整数据、问答协议和评测器；Ownward 不接触答案、证据标签或评测函数。官方问答准确率与 Ownward 在固定等价条件下测得的检索时延、返回上下文量、资源和外部调用成本分开报告；只有数据版本、Reader、裁判、提示、检索预算和计时边界等价的成绩才可直接比较，项目自测指标不得冒充官方指标。
