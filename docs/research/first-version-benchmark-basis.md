...
公开质量与时效以 LongMemEval-V2 Small 的官方评测器和 LAFS 前沿为主。它直接评估经验、流程、环境陷阱和前提适用性，并同时计入回答准确率与在线记忆查询时延。冻结时公开的 AgentRunbook-C V2 高质量点为 75.61% / 130.54 秒，Ownward 必须在相同读者、裁判、数据和预算下进入或超过该前沿。
