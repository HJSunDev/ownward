...
CANDIDATE_RECEIPT_SCHEMA = "ownward.kernel-iteration-v2-candidate/v1"
CANDIDATE_POLICY = "bounded-source-continuity-diversity/v1"
...
def prepare(
    ...
    *,
    ...
    """Build one self-contained, non-formal V2 artifact from exact workspace content."""
    ...
    paths = (receipt_path, subject_path, candidate_config_path, binary_path, composition_path, overlay_path)
    if any(path.exists() for path in (*paths, embedding_path)):
        _require(resume and all(path.is_file() for path in paths) and embedding_path.is_dir(), "V2 候选准备现场不完整；禁止覆盖或宽松复用")
        receipt = _load_json(receipt_path)
        subject = evidence.validate_v2_subject(evidence.load_contract(suite_root), _load_json(subject_path))
        _require(receipt.get("schema") == CANDIDATE_RECEIPT_SCHEMA and receipt.get("subject_identity") == subject["identity"], "V2 候选准备收据漂移")
        _require(receipt.get("builder_sha256") == evidence.file_sha256(Path(__file__).resolve()), "V2 候选构建器身份漂移")
        _require(receipt.get("binary_sha256") == evidence.file_sha256(binary_path), "V2 候选二进制漂移")
        _require(receipt.get("composition_sha256") == evidence.file_sha256(composition_path), "V2 候选组合制品漂移")
        _require(receipt.get("overlay_sha256") == evidence.file_sha256(overlay_path), "V2 候选构建覆盖身份漂移")
        _require(receipt.get("embedding_manifest_sha256") == evidence.file_sha256(embedding_path / "manifest.json"), "V2 候选向量清单漂移")
        config = _load_json(candidate_config_path)
        _require(Path(str(_mapping(config, "candidate").get("binary", ""))).resolve() == binary_path, "V2 候选执行配置错绑")
        _require(Path(str(_mapping(config, "candidate").get("embedding_bundle_dir", ""))).resolve() == embedding_path, "V2 候选向量制品错绑")
    ...
    template["name"] = "v2-fragment-complete-candidate"
    ...
    kernel["config"] = {**_mapping(kernel, "config"), "evidence_selection": CANDIDATE_POLICY}
    content = kernel.get("content")
    _require(isinstance(content, list), "当前内核组件内容无效")
    overlay_relative = "manifests/kernel-candidates/v2/fragment-complete/core-evidence.go.overlay"
    composition_embed_relative = "manifests/kernel-candidates/v2/fragment-complete/composition-embed.go.overlay"
    candidate_relative = "internal/kernelv2candidate/evidence.go"
    ...
    core_evidence["path"] = overlay_relative
    content.append({"name": "v2-evidence-continuity", "path": candidate_relative, "sha256": ""})
    ...
    _require(sealed.returncode == 0 and composition_path.is_file(), f"封存 V2 候选组合失败: {sealed.stderr.strip()}")
    ...
    _require(isinstance(sealed_kernel, dict) and _mapping(sealed_kernel, "config").get("evidence_selection") == CANDIDATE_POLICY, "V2 候选组合没有封存证据连续性策略")
    dependencies = {item["role"]: item["identity"] for item in sealed_kernel.get("dependencies", [])}
    _require(set(dependencies) == {"authority-substrate", "product-rules", "semantic", "vector"}, "V2 候选内核直接依赖不完整")
    generation_identity = evidence.canonical_sha256({
        ...
        "evidence_selection": CANDIDATE_POLICY,
        "direct_dependencies": dict(sorted(dependencies.items())),
    })
    evidence.atomic_json(overlay_path, {
        "Replace": {
            str((repository / "internal" / "core" / "evidence.go").resolve()): str((repository / overlay_relative).resolve()),
    ...
    _require(build.returncode == 0 and binary_path.is_file(), f"构建 V2 候选失败: {build.stderr.strip()}")
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise validation.KernelIterationValidationError(f"无法读取 V2 候选制品 {path}: {error}") from error
    _require(isinstance(value, dict), f"V2 候选制品不是对象: {path}")
    return value


def _mapping(value: dict[str, Any], name: str) -> dict[str, Any]:
    result = value.get(name)
    _require(isinstance(result, dict), f"V2 候选缺少对象: {name}")
    return result
