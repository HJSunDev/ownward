...
TRANSFORM_SCHEMA = "ownward.kernel-candidate-source-transform/v1"
SHARED_VECTOR_RUNTIME = {"threads": 6, "threads_batch": 6, "parallel": 2}
...
def prepare(
    ...
    *,
    ...
    _require(output_root.is_relative_to(repository / ".tmp"), "V2 检索时延候选制品只能写入仓库非正式 .tmp 边界")
    ...
    paths = receipt_path, subject_path, candidate_config_path, binary_path, composition_path, overlay_path, generated_service_path, generated_runtime_path
    if any(path.exists() for path in (*paths, embedding_path)):
        _require(resume and all(path.is_file() for path in paths) and embedding_path.is_dir(), "V2 检索时延候选现场不完整；禁止覆盖或宽松复用")
        receipt = _load_json(receipt_path)
        subject = evidence.validate_v2_subject(evidence.load_contract(suite_root), _load_json(subject_path))
        _require(receipt.get("schema") == CANDIDATE_RECEIPT_SCHEMA and receipt.get("subject_identity") == subject["identity"], "V2 检索时延候选收据漂移")
        checks = {
            ...
            "embedding_runtime_source_sha256": evidence.file_sha256(generated_runtime_path),
            "embedding_runtime_base_source_sha256": evidence.file_sha256(repository / "internal" / "embedding" / "llama.go"),
            "embedding_runtime_transform_sha256": evidence.file_sha256(repository / "manifests" / "kernel-candidates" / "v2" / "retrieval-latency" / "managed-runtime-transform.json"),
        }
        for field, actual in checks.items():
            _require(receipt.get(field) == actual, f"V2 检索时延候选制品漂移: {field}")
    ...
    _render_transformed_source(repository, transform_path, generated_service_path)
    runtime_transform_relative = "manifests/kernel-candidates/v2/retrieval-latency/managed-runtime-transform.json"
    runtime_transform_path = repository / runtime_transform_relative
    _render_transformed_source(repository, runtime_transform_path, generated_runtime_path)

    current_path = repository / "manifests" / "compositions" / "v1" / "current-collaborative.json"
    template = _load_json(current_path)
    template["name"] = "v2-retrieval-latency-candidate"
    ...
    vector_content = vector.get("content")
    _require(isinstance(vector_content, list), "当前向量组件内容无效")
    vector_content.append({"name": "v2-managed-runtime-transform", "path": runtime_transform_relative, "sha256": ""})
    vector["config"] = {**_mapping(vector, "config"), "runtime": dict(SHARED_VECTOR_RUNTIME), "scope": "non-formal-comparison"}
    ...
    _require(sealed.returncode == 0 and composition_path.is_file(), f"封存 V2 检索时延候选组合失败: {sealed.stderr.strip()}")
    ...
    _require(isinstance(sealed_kernel, dict), "V2 检索时延候选缺少封存内核")
    sealed_config = _mapping(sealed_kernel, "config")
    _require(
        sealed_config.get("evidence_selection") == first_candidate.CANDIDATE_POLICY
        and sealed_config.get("evidence_source_scheduling") == multisource_candidate.CANDIDATE_POLICY
        and sealed_config.get("semantic_query_strategy") == CANDIDATE_POLICY,
        "V2 检索时延候选没有封存三条策略",
    )
    dependencies = {item["role"]: item["identity"] for item in sealed_kernel.get("dependencies", [])}
    _require(set(dependencies) == {"authority-substrate", "product-rules", "semantic", "vector"}, "V2 检索时延候选内核直接依赖不完整")
    ...
    evidence.atomic_json(overlay_path, {"Replace": {
        ...
        str((repository / "internal" / "embedding" / "llama.go").resolve()): str(generated_runtime_path),
    ...
    ldflags = (
        ...
        f"-X github.com/HJSunDev/ownward/internal/embedding.managedRuntimeThreads={SHARED_VECTOR_RUNTIME['threads']} "
        f"-X github.com/HJSunDev/ownward/internal/embedding.managedRuntimeParallel={SHARED_VECTOR_RUNTIME['parallel']}"
    )
    ...
    _require(build.returncode == 0 and binary_path.is_file(), f"构建 V2 检索时延候选失败: {build.stderr.strip()}")
    ...
    subject_content = {
        ...
        "artifacts": {
            ...
            "generated-runtime": evidence.file_sha256(generated_runtime_path),
            "source-transform": evidence.file_sha256(transform_path),
            "runtime-source-transform": evidence.file_sha256(runtime_transform_path),
        },
    }
    subject_identity = evidence.canonical_sha256(subject_content)
    evidence.atomic_json(subject_path, {
        **subject_content,
        "name": "v2-retrieval-latency-candidate",
    ...
    receipt_content = {
        ...
        "embedding_runtime_source_sha256": evidence.file_sha256(generated_runtime_path),
        "embedding_runtime_base_source_sha256": evidence.file_sha256(repository / "internal" / "embedding" / "llama.go"),
        "embedding_runtime_transform_sha256": evidence.file_sha256(runtime_transform_path),
        "embedding_runtime_configuration": dict(SHARED_VECTOR_RUNTIME),
        "embedding_manifest_sha256": evidence.file_sha256(embedding_path / "manifest.json"),
...
def _render_transformed_source(repository: Path, transform_path: Path, output_path: Path) -> None:
    transform = _load_json(transform_path)
    _require(transform.get("schema") == TRANSFORM_SCHEMA, "V2 检索时延候选源码转换合同无效")
    source_path = (repository / str(transform.get("source", ""))).resolve()
    _require(source_path.is_relative_to(repository.resolve()) and source_path.is_file(), "V2 检索时延候选源码转换来源无效")
    value = source_path.read_text(encoding="utf-8")
    replacements = transform.get("replacements")
    _require(isinstance(replacements, list) and replacements, "V2 检索时延候选源码转换步骤为空")
    for replacement in replacements:
        _require(isinstance(replacement, dict), "V2 检索时延候选源码转换步骤无效")
        before, after, count = replacement.get("before"), replacement.get("after"), replacement.get("count")
        _require(isinstance(before, str) and isinstance(after, str) and isinstance(count, int) and count > 0, "V2 检索时延候选源码转换参数无效")
        _require(value.count(before) == count, f"V2 检索时延候选源码转换来源漂移: {replacement.get('name', '')}")
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise validation.KernelIterationValidationError(f"无法读取 V2 检索时延候选制品 {path}: {error}") from error
    _require(isinstance(value, dict), f"V2 检索时延候选制品不是对象: {path}")
    return value


def _mapping(value: dict[str, Any], name: str) -> dict[str, Any]:
    result = value.get(name)
    _require(isinstance(result, dict), f"V2 检索时延候选缺少对象: {name}")
    return result
