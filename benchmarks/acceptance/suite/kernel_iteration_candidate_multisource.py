...
import kernel_iteration_stage4_multisource as multisource
import kernel_iteration_validation as validation


CANDIDATE_RECEIPT_SCHEMA = "ownward.kernel-iteration-v2-candidate/v2"
CANDIDATE_POLICY = "bounded-source-breadth-before-repeated-depth/v1"
...
def prepare(
    ...
    *,
    ...
    """Build the second independent V2 candidate from exact sealed content."""
    repository = suite_root.resolve().parents[2]
    output_root = output_root.resolve()
    _require(output_root.is_relative_to(repository / ".tmp"), "V2 候选制品只能写入仓库非正式 .tmp 边界")
    runtime = validation.validate_execution_config(suite_root, execution_config_path.resolve())
    multisource.load_route(suite_root)
    ...
    paths = (
        receipt_path, subject_path, candidate_config_path, binary_path,
        composition_path, overlay_path, generated_service_path,
    )
    if any(path.exists() for path in (*paths, embedding_path)):
        _require(
            resume and all(path.is_file() for path in paths) and embedding_path.is_dir(),
            "V2 多来源候选准备现场不完整；禁止覆盖或宽松复用",
        )
        ...
        _require(
            ...
            and receipt.get("subject_identity") == subject["identity"],
            "V2 多来源候选准备收据漂移",
        )
        checks = {
        ...
        for field, actual in checks.items():
            _require(receipt.get(field) == actual, f"V2 多来源候选制品漂移: {field}")
        config = _load_json(candidate_config_path)
        _require(Path(str(_mapping(config, "candidate").get("binary", ""))).resolve() == binary_path, "V2 多来源候选执行配置错绑")
        _require(Path(str(_mapping(config, "candidate").get("embedding_bundle_dir", ""))).resolve() == embedding_path, "V2 多来源候选向量制品错绑")
        return {**receipt, "reused": True, "subject_manifest": str(subject_path), "execution_config": str(candidate_config_path)}

    output_root.mkdir(parents=True, exist_ok=True)
    transform_relative = "manifests/kernel-candidates/v2/multisource-complete/service-transform.json"
    ...
    template["name"] = "v2-multisource-complete-candidate"
    ...
    kernel["config"] = {
        **_mapping(kernel, "config"),
        ...
        "evidence_source_scheduling": CANDIDATE_POLICY,
    }
    ...
    content.extend([
        ...
        {"name": "v2-service-transform", "path": transform_relative, "sha256": ""},
    ])
    ...
    _require(sealed.returncode == 0 and composition_path.is_file(), f"封存 V2 多来源候选组合失败: {sealed.stderr.strip()}")
    ...
    _require(isinstance(sealed_kernel, dict), "V2 多来源候选缺少封存内核")
    sealed_config = _mapping(sealed_kernel, "config")
    _require(
        sealed_config.get("evidence_selection") == first_candidate.CANDIDATE_POLICY
        and sealed_config.get("evidence_source_scheduling") == CANDIDATE_POLICY,
        "V2 多来源候选没有封存两条证据策略",
    )
    dependencies = {item["role"]: item["identity"] for item in sealed_kernel.get("dependencies", [])}
    _require(set(dependencies) == {"authority-substrate", "product-rules", "semantic", "vector"}, "V2 多来源候选内核直接依赖不完整")
    ...
    _require(build.returncode == 0 and binary_path.is_file(), f"构建 V2 多来源候选失败: {build.stderr.strip()}")
    ...
    evidence.atomic_json(subject_path, {
        **subject_content,
        "name": "v2-multisource-complete-candidate",
...
def _render_transformed_source(repository: Path, transform_path: Path, output_path: Path) -> None:
    ...
    _require(transform.get("schema") == TRANSFORM_SCHEMA, "V2 候选源码转换合同无效")
    source_relative = str(transform.get("source", ""))
    source_path = (repository / source_relative).resolve()
    _require(source_path.is_relative_to(repository.resolve()) and source_path.is_file(), "V2 候选源码转换来源无效")
    value = source_path.read_text(encoding="utf-8")
    replacements = transform.get("replacements")
    _require(isinstance(replacements, list) and replacements, "V2 候选源码转换步骤为空")
    for replacement in replacements:
        _require(isinstance(replacement, dict), "V2 候选源码转换步骤无效")
        before, after, count = replacement.get("before"), replacement.get("after"), replacement.get("count")
        _require(isinstance(before, str) and isinstance(after, str) and isinstance(count, int) and count > 0, "V2 候选源码转换参数无效")
        _require(value.count(before) == count, f"V2 候选源码转换来源漂移: {replacement.get('name', '')}")
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise validation.KernelIterationValidationError(f"无法读取 V2 多来源候选制品 {path}: {error}") from error
    _require(isinstance(value, dict), f"V2 多来源候选制品不是对象: {path}")
    return value


def _mapping(value: dict[str, Any], name: str) -> dict[str, Any]:
    result = value.get(name)
    _require(isinstance(result, dict), f"V2 多来源候选缺少对象: {name}")
    return result
